...
- A. 创建一个 Amazon Elastic Container Service (Amazon ECS) 集群。将处理配置为 AWS Fargate 任务。将容器选择逻辑提取为一个 Amazon EventBridge 规则，启动适当的 Fargate 任务。配置 EventBridge 规则在 EFS 文件系统添加文件时运行。
- B. 创建一个 Amazon Elastic Container Service (Amazon ECS) 集群。将处理配置为 AWS Fargate 任务。更新并容器化容器选择逻辑，作为启动适当 Fargate 任务的 Fargate 服务。配置 EFS 事件通知以在向 EFS 文件系统添加文件时调用 Fargate 服务。
- C. 创建一个 Amazon Elastic Container Service (Amazon ECS) 集群。将处理配置为 AWS Fargate 任务。将容器选择逻辑提取为一个 AWS Lambda 函数，启动适当的 Fargate 任务。迁移文件上传的存储到 Amazon S3 存储桶。更新处理代码以使用 Amazon S3。配置 S3 事件通知，当创建对象时调用 Lambda 函数。
- D. 为处理创建 AWS Lambda 容器镜像。配置 Lambda 函数使用容器镜像。将容器选择逻辑提取为一个决策 Lambda 函数，调用适当的 Lambda 处理函数。迁移文件上传的存储到 Amazon S3 存储桶。更新处理代码以使用 Amazon S3。配置 S3 事件通知，当创建对象时调用决策 Lambda 函数。
...
- A. 将 AWS CloudFormation 模板上传到 Amazon S3。允许 QA 部门的用户假定经理的角色，并添加一个策略来限制对模板及其创建的资源的权限。培训用户从 CloudFormation 控制台启动模板。
- B. 从环境模板创建 AWS 服务目录产品。使用现有角色添加产品启动约束。允许 QA 部门的用户仅使用 AWS 服务目录 API。培训用户从 AWS 服务目录控制台启动模板。
- C. 将 AWS CloudFormation 模板上传到 Amazon S3。允许 QA 部门的用户使用 CloudFormation 和 S3 API，条件限制对模板及其创建的资源的权限。培训用户从 CloudFormation 控制台启动模板。
- D. 从环境模板创建 AWS Elastic Beanstalk 应用程序。允许 QA 部门的用户仅使用 Elastic Beanstalk 权限。培训用户使用 Elastic Beanstalk CLI 启动 Elastic Beanstalk 环境，将现有角色作为服务角色传递给环境。

---

### 单选题 203/306

一家娱乐公司在一组 Linux Amazon EC2 实例上托管票务服务，这些实例位于自动扩展组中。票务服务使用一个价格文件。价格文件存储在具有 S3 标准存储的 Amazon S3 存储桶中。一个由第三方托管的中央定价解决方案会更新价格文件。价格文件每 1-15 分钟更新一次，包含数千个项目。当实例启动时，价格文件会下载到每个 EC2 实例。EC2 实例偶尔使用过时的定价信息，可能导致客户收费错误。哪种解决方案将以最具成本效益的方式解决这个问题？

- A. 创建一个 AWS Lambda 函数，每次更新价格文件时，使用新价格更新 Amazon DynamoDB 表。更新票务服务以使用 DynamoDB 查询价格。
- B. 创建一个 AWS Lambda 函数，每次更新价格文件时，使用 Amazon Elastic File System (Amazon EFS) 文件共享更新价格文件。更新票务服务以使用 Amazon EFS 访问价格文件。
- C. 将 Mountpoint for Amazon S3 加载到 EC2 实例的 AMI 上。配置 Mountpoint for Amazon S3 挂载包含价格文件的 S3 存储桶。更新票务服务指向挂载点和路径以访问 S3 对象。
- D. 创建一个 Amazon Elastic Block Store (Amazon EBS) 卷。使用 EBS Multi-Attach 将卷附加到每个 EC2 实例。当新 EC2 实例启动时，配置新实例更新 EBS 卷上的价格文件。更新票务服务指向新的本地源。
...
- A. 使用预测性扩展策略。使用实例维护策略来运行用户数据脚本。将默认实例预热时间设置为 0 秒。
- B. 使用动态扩展策略。使用生命周期挂钩来运行用户数据脚本。将默认实例预热时间设置为 0 秒。
- C. 使用预测性扩展策略。为自动扩展组启用热池。使用实例维护策略来运行用户数据脚本。
- D. 使用动态扩展策略。为自动扩展组启用热池。使用生命周期挂钩来运行用户数据脚本。
...
一家公司计划将许多虚拟机从本地环境迁移到 AWS。公司需要在迁移之前对本地环境进行初步评估，可视化运行在虚拟机上的应用程序之间的依赖关系，并提供对本地环境的评估报告。为了获取这些信息，公司启动了一个迁移评估请求。公司有能力在本地环境中安装收集器软件，没有任何限制。哪种解决方案将以最小的运营开销为公司提供所需信息？

- A. 在每个本地虚拟机上安装 AWS 应用程序发现代理。在数据收集期结束后，使用 AWS 迁移中心查看应用程序依赖关系。从迁移中心下载快速洞察评估报告。
- B. 在每个本地虚拟机上安装迁移评估收集器。在数据收集期结束后，使用迁移评估查看应用程序依赖关系。下载并导出迁移评估中发现的服务器列表。将列表上传到 Amazon QuickSight。当 QuickSight 报告生成后，下载快速洞察评估报告。
- C. 在本地环境中设置 AWS 应用程序发现服务无代理收集器。在数据收集期结束后，使用 AWS 迁移中心查看应用程序依赖关系。从应用程序发现服务中导出发现的服务器列表。将列表上传到迁移评估。当迁移评估报告生成后，下载快速洞察评估。
- D. 在本地环境中设置迁移评估收集器。在每个虚拟机上安装 AWS 应用程序发现代理。在数据收集期结束后，使用 AWS 迁移中心查看应用程序依赖关系。从迁移评估下载快速洞察评估报告。

---

### 单选题 216/306

一家公司最近成功完成了 Amazon WorkSpaces 的概念验证。解决方案架构师需要使解决方案在两个 AWS 区域中高度可用。Amazon WorkSpaces 部署在一个故障转移区域，并且在 Amazon Route 53 中部署了一个托管区域。解决方案架构师应该如何配置解决方案的高可用性？

- A. 在主区域和故障转移区域创建连接别名。将连接别名与每个区域的目录关联。创建 Route 53 故障转移路由策略。将评估目标健康设置为是。
- B. 在主区域和故障转移区域创建连接别名。将连接别名与主区域的目录关联。创建 Route 53 多值答案路由策略。
- C. 在主区域创建连接别名。将连接别名与主区域的目录关联。创建 Route 53 加权路由策略。
- D. 在主区域创建连接别名。将连接别名与故障转移区域的目录关联。创建 Route 53 故障转移路由策略。将评估目标健康设置为是。

---

### 单选题 217/306

一家公司在 AWS 云中运行多个应用程序。这些应用程序专门用于公司内的独立业务单位。公司在 AWS 组织中的多个 AWS 账户中运行应用程序的组件。公司组织中的每个云资源都有一个名为 BusinessUnit 的标签。每个标签已经有了适当的业务单位名称值。公司需要将云成本分配给不同的业务单位。公司还需要为每个业务单位可视化云成本。哪种解决方案将满足这些要求？

- A. 在组织的管理账户中，创建一个名为 BusinessUnit 的成本分配标签。在管理账户中，创建一个 Amazon S3 存储桶和一个 AWS 成本和使用情况报告（AWS CUR）。将 S3 存储桶配置为 AWS CUR 的目标。从管理账户中，使用 Amazon Athena 查询 AWS CUR 数据。使用 Amazon QuickSight 进行可视化。
- B. 在每个成员账户中，创建一个名为 BusinessUnit 的成本分配标签。在组织的管理账户中，创建一个 Amazon S3 存储桶和一个 AWS 成本和使用情况报告（AWS CUR）。将 S3 存储桶配置为 AWS CUR 的目标。创建一个 Amazon CloudWatch 仪表板进行可视化。
- C. 在组织的管理账户中，创建一个名为 BusinessUnit 的成本分配标签。在每个成员账户中，创建一个 Amazon S3 存储桶和一个 AWS 成本和使用情况报告（AWS CUR）。将每个 S3 存储桶配置为其各自的 AWS CUR 的目标。
- D. 在每个成员账户中，创建一个名为 BusinessUnit 的成本分配标签。在每个成员账户中，创建一个 Amazon S3 存储桶和一个 AWS 成本和使用情况报告（AWS CUR）。将每个 S3 存储桶配置为其各自的 AWS CUR 的目标。从管理账户中，使用 Amazon Athena 查询 AWS CUR 数据。使用 Amazon QuickSight 进行可视化。

---

### 单选题 218/306

一家公司需要使用带 Amazon S3 存储桶的 AWS Transfer Family SFTP 启用服务器从第三方数据供应商那里接收更新。数据使用 Pretty Good Privacy（PGP）加密。公司需要一个解决方案，在公司接收数据后能自动解密数据。解决方案架构师将使用 Transfer Family 管理的工作流。公司已经使用允许访问 AWS Secrets Manager 和 S3 存储桶的 IAM 策略创建了一个 IAM 服务角色。角色的信任关系允许 transfer.amazonaws.com 服务承担该角色。解决方案架构师接下来应该做什么来完成自动解密的解决方案？

- A. 在 Secrets Manager 中存储 PGP 公钥。在 Transfer Family 管理的工作流中添加一个名义步骤来解密文件。在名义步骤中配置 PGP 加密参数。将工作流与 Transfer Family 服务器关联。
- B. 在 Secrets Manager 中存储 PGP 私钥。在 Transfer Family 管理的工作流中添加一个异常处理步骤来解密文件。在异常处理程序中配置 PGP 加密参数。将工作流与 SFTP 用户关联。
- C. 在 Secrets Manager 中存储 PGP 私钥。在 Transfer Family 管理的工作流中添加一个名义步骤来解密文件。在名义步骤中配置 PGP 解密参数。将工作流与 Transfer Family 服务器关联。
- D. 在 Secrets Manager 中存储 PGP 公钥。在 Transfer Family 管理的工作流中添加一个异常处理步骤来解密文件。在异常处理程序中配置 PGP 解密参数。将工作流与 SFTP 用户关联。

---

### 单选题 219/306

一位解决方案架构师正在从现有的手动创建的非生产 AWS 环境创建一个 AWS CloudFormation 模板。CloudFormation 模板可以根据需要被销毁和重新创建。环境包含一个 Amazon EC2 实例。EC2 实例有一个实例配置文件，该实例使用它在父账户中承担一个角色。解决方案架构师在 CloudFormation 模板中重新创建了该角色，并使用了相同的角色名称。当 CloudFormation 模板在子账户中启动时，EC2 实例不能再在父账户中承担该角色，因为权限不足。解决方案架构师应该采取什么措施来解决这个问题？

- A. 在父账户中，编辑 EC2 实例需要承担的角色的信任策略。确保现有的允许 sts:AssumeRole 操作的声明中的 target role ARN 是正确的。保存信任策略。
- B. 在父账户中，编辑 EC2 实例需要承担的角色的信任策略。为子账户的根主体添加一个允许 sts:AssumeRole 操作的声明。保存信任策略。
- C. 再次更新 CloudFormation 堆栈。指定只有 CAPABILITY_NAMED_IAM 能力。
- D. 再次更新 CloudFormation 堆栈。指定 IAM 能力 CAPABILITY_IAM 和命名 IAM 能力 CAPABILITY_NAMED_IAM。

---

### 单选题 220/306

一家公司需要提高其票务应用程序的可靠性。该应用程序运行在一个 Amazon Elastic Container Service（Amazon ECS）集群上。公司使用 Amazon CloudFront 为应用程序提供服务。ECS 集群中的单个 ECS 服务是 CloudFront 分发的起源。应用程序只允许特定数量的活跃用户进入购票流程。这些用户通过其 JSON Web Token（JWT）中的加密属性进行识别。所有其他用户都被重定向到一个等候室模块，直到有可用的购买容量。应用程序正在经历高负载。等候室模块按设计工作，但是对等候室的负载正在干扰应用程序的可用性。这种干扰对应用程序的票务销售交易产生了负面影响。哪种解决方案将在高负载期间为票务销售交易提供最大的可靠性？

- A. 在 ECS 集群中为等候室创建一个单独的服务。使用单独的扩展配置。确保票务服务使用 JWT 信息并适当地将请求转发到等候室服务。
- B. 将应用程序移动到 Amazon Elastic Kubernetes Service（Amazon EKS）集群。将等候室模块拆分成一个与票务 pod 分开的 pod。使票务 pod 成为 StatefulSet 的一部分。确保票务 pod 使用 JWT 信息并适当地将请求转发到等候室 pod。
- C. 在 ECS 集群中为等候室创建一个单独的服务。使用单独的扩展配置。创建一个 CloudFront 函数，检查 JWT 信息并适当地将请求转发到票务服务或等候室服务。
- D. 将应用程序移动到 Amazon Elastic Kubernetes Service（Amazon EKS）集群。将等候室模块拆分成一个与票务 pod 分开的 pod。使用 AWS App Mesh，通过为 Kubernetes 配置 App Mesh 控制器。启用 mTLS 认证和票务 pod 与等候室 pod 之间的服务到服务认证。确保票务 pod 使用 JWT 信息并适当地将请求转发到等候室 pod。

---

### 多选题 221/306

一家公司在本地数据中心运行一个三层的 Web 应用程序。前端由 Apache Web 服务器提供服务，中间层是一个单体 Java 应用程序，存储层是一个 PostgreSQL 数据库。在最近的一次市场推广中，客户无法通过应用程序下订单，因为应用程序崩溃了。分析显示所有三层都过载了。应用程序变得无响应，数据库由于读取操作达到了容量限制。该公司已经在近期安排了几个类似的推广活动。解决方案架构师必须为迁移到 AWS 制定一个计划来解决这些问题。该解决方案必须最大限度地提高可扩展性，同时最小化操作工作量。哪种组合的步骤将满足这些要求？（选择三个）

- A. 重构前端，以便静态资源可以托管在 Amazon S3 上。使用 Amazon CloudFront 为客户端提供前端服务。将前端连接到 Java 应用程序。
- B. 将前端的 Apache Web 服务器重新托管在 Amazon EC2 实例上，这些实例位于自动扩展组中。在自动扩展组前使用负载均衡器。使用 Amazon Elastic File System（Amazon EFS）托管 Apache Web 服务器所需的静态资源。
- C. 在包含自动扩展的 AWS Elastic Beanstalk 环境中重新托管 Java 应用程序。
- D. 重构 Java 应用程序，开发一个 Docker 容器来运行 Java 应用程序。使用 AWS Fargate 托管容器。
- E. 使用 AWS Database Migration Service（AWS DMS）将 PostgreSQL 数据库重新部署到 Amazon Aurora PostgreSQL 数据库。使用 Aurora 自动扩展来读取副本。
- F. 重新托管 PostgreSQL 数据库在 Amazon EC2 实例上，该实例的内存是本地服务器的两倍。

---

### 单选题 222/306

一家软件开发公司的多名工程师正在远程工作。公司在 Amazon EC2 实例上运行 Active Directory Domain Services (AD DS)。公司的安全政策规定，部署在 VPC 中的所有内部非公开服务必须通过 VPN 访问。必须使用多因素认证(MFA)访问 VPN。解决方案架构师应如何满足这些要求？

- A. 创建一个 AWS 站点到站点 VPN 连接。配置 VPN 与 AD DS 的集成。使用启用了 MFA 支持的 Amazon WorkSpaces 客户端建立 VPN 连接。
- B. 创建一个 AWS 客户端 VPN 端点。为与 AD DS 的集成创建一个 AD 连接器目录。为 AD 连接器启用 MFA。使用 AWS 客户端 VPN 建立 VPN 连接。
- C. 使用 AWS VPN CloudHub 创建多个 AWS 站点到站点 VPN 连接。配置 AWS VPN CloudHub 与 AD DS 的集成。使用 AWS Copilot 建立 VPN 连接。
- D. 创建一个 Amazon WorkLink 端点。配置 Amazon WorkLink 与 AD DS 的集成。在 Amazon WorkLink 中启用 MFA。使用 AWS 客户端 VPN 建立 VPN 连接。

---

### 单选题 223/306

一家公司正计划从本地数据中心迁移到 AWS 云。公司计划使用在 AWS Organizations 中管理的多个 AWS 账户。公司最初将创建少量账户，并根据需要添加账户。解决方案架构师必须设计一个解决方案，以在所有 AWS 账户中启用 AWS CloudTrail。哪种解决方案在操作上最有效？

- A. 创建一个 AWS Lambda 函数，在组织中的所有 AWS 账户中创建一个新的 CloudTrail 轨迹。使用 Amazon EventBridge 中的计划动作每天调用 Lambda 函数。
- B. 在组织的管理账户中创建一个新的 CloudTrail 轨迹。配置轨迹记录组织中所有 AWS 账户的所有事件。
- C. 在组织中的所有 AWS 账户中创建一个新的 CloudTrail 轨迹。每当创建一个新账户时，就创建新的轨迹。定义一个 SCP，防止删除或修改轨迹。将 SCP 应用于根 OU。
- D. 创建一个 AWS Systems Manager Automation runbook，在组织中的所有 AWS 账户中创建 CloudTrail 轨迹。使用 Systems Manager State Manager 调用自动化。

---

### 多选题 224/306

一家公司使用 AWS Organizations。公司在集中网络账户中运行两个防火墙设备。每个防火墙设备都在手动配置的高可用 Amazon EC2 实例上运行。一个传输网关连接来自集中网络账户的 VPC 和成员账户的 VPC。每个防火墙设备使用静态私有 IP 地址，然后用于将流量从成员账户路由到互联网。在最近的一个事件中，一个配置错误的脚本启动了两个防火墙设备的终止。在重建防火墙设备期间，公司编写了一个新的脚本以在启动时配置防火墙设备。公司希望现代化防火墙设备的部署。防火墙设备需要具备水平扩展的能力，以处理网络扩展时增加的流量。公司必须继续使用防火墙设备以符合公司政策。防火墙设备的提供商已确认最新版本的防火墙代码将与所有 AWS 服务兼容。解决方案架构师应该推荐哪些组合的步骤，以最具成本效益的方式满足这些要求？（选择三项）

- A. 在集中网络账户中部署一个网关负载均衡器。设置一个使用 AWS PrivateLink 的终端节点服务。
- B. 在集中网络账户中部署一个网络负载均衡器。设置一个使用 AWS PrivateLink 的终端节点服务。
- C. 创建一个自动扩展组和一个启动模板，使用新脚本作为用户数据来配置防火墙设备。创建一个使用实例目标类型的目标组。
- D. 创建一个自动扩展组。配置一个 AWS Launch Wizard 部署，使用新脚本作为用户数据来配置防火墙设备。创建一个使用 IP 目标类型的目标组。
- E. 在每个成员账户中创建 VPC 终端节点。更新路由表以指向 VPC 终端节点。
- F. 在集中网络账户中创建 VPC 终端节点。更新每个成员账户中的路由表，指向 VPC 终端节点。

---

### 单选题 225/306

一家公司希望使用 Amazon WorkSpaces 结合瘦客户端设备来替换陈旧的桌面设备。员工使用桌面设备访问与临床试验数据相关的应用程序。公司安全政策规定，对应用程序的访问必须限制在只有公司分支机构位置。公司正在考虑在未来 6 个月内增加一个额外的分支机构。哪种解决方案以最高的运营效率满足这些要求？

- A. 创建一个 IP 访问控制组规则，列出分支机构的公共地址。将 IP 访问控制组与 WorkSpaces 目录关联。
- B. 使用 AWS 防火墙管理器创建一个带有 IPSet 的 Web ACL 规则，列出分支机构位置的公共地址。将 Web ACL 与 WorkSpaces 目录关联。
- C. 使用 AWS 证书管理器(ACM)向部署在分支机构位置的机器发行受信任的设备证书。在 WorkSpaces 目录上启用受限访问。
- D. 创建一个自定义 WorkSpace 镜像，配置 Windows 防火墙以限制对分支机构公共地址的访问。使用该镜像部署 WorkSpaces。

---

### 多选题 226/306

一家公司希望在其本地基础设施和 AWS 之间建立专用连接。公司正在为其账户 VPC 设置 1 Gbps AWS Direct Connect 连接。架构包括一个传输网关和一个 Direct Connect 网关，用于连接多个 VPC 和本地基础设施。公司必须使用 Direct Connect 连接通过传输 VIF 连接到 VPC 资源。哪些组合的步骤将满足这些要求？（选择两项）

- A. 将 1 Gbps Direct Connect 连接更新为 10 Gbps。
- B. 在传输 VIF 上广播本地网络前缀。
- C. 从 Direct Connect 网关向传输 VIF 广播 VPC 前缀到本地网络。
- D. 更新 Direct Connect 连接的 MACsec 加密模式属性为必须加密。
- E. 将 MACsec 连接密钥名称/连接关联密钥(CKN/CAK)对与 Direct Connect 连接关联。

---

### 单选题 227/306

一家公司在多个 AWS 账户中部署工作负载。每个账户都有一个 VPC，VPC 流量日志以文本日志格式发布到一个集中的 Amazon S3 存储桶。每个日志文件都使用 gzip 压缩。公司必须无限期保留日志文件。安全工程师偶尔使用 Amazon Athena 分析日志，但随着摄取日志数量的增加，查询性能逐渐下降。解决方案架构师必须提高日志分析的性能并减少 VPC 流量日志使用的存储空间。哪种解决方案在提高性能方面提供了最大的改进？

- A. 创建一个 AWS Lambda 函数来解压缩 gzip 文件，并使用 bzip2 压缩重新压缩文件。为 S3 存储桶订阅 Lambda 函数的 s3:ObjectCreated:Put S3 事件通知。
- B. 为 S3 存储桶启用 S3 传输加速。创建一个 S3 生命周期配置，将文件在上传后立即移动到 S3 智能分层存储类别。
- C. 更新 VPC 流量日志配置，将文件存储为 Apache Parquet 格式。为日志文件指定每小时分区。
- D. 创建一个新的 Athena 工作组，不设置数据使用控制限制。使用 Athena 引擎版本 2。

---

### 单选题 228/306

一家公司希望将虚拟的 Microsoft 工作负载从本地数据中心迁移到 AWS。公司已成功在 AWS 上测试了一些示例工作负载。公司还创建了一个 AWS 站点到站点 VPN 连接到 VPC。解决方案架构师需要为所有工作负载从数据中心迁移生成一份总拥有成本(TCO)报告。每个虚拟机在数据中心上都启用了简单网络管理协议(SNMP)。公司不能在数据中心中添加更多的虚拟机，也不能在虚拟机上安装额外的软件。发现数据必须自动导入到 AWS 迁移中心。哪种解决方案能满足这些要求？

- A. 使用 AWS 应用程序迁移服务无代理服务和 AWS 迁移中心策略建议来生成 TCO 报告。
- B. 启动一个 Windows Amazon EC2 实例。在 EC2 实例上安装迁移评估无代理收集器。配置迁移评估生成 TCO 报告。
- C. 启动一个 Windows Amazon EC2 实例。在 EC2 实例上安装迁移评估无代理收集器。配置迁移中心生成 TCO 报告。
- D. 在 VPC 内使用 AWS 迁移就绪评估工具。配置迁移评估生成 TCO 报告。

---

### 单选题 229/306

一家公司使用 AWS Organizations 管理多个 AWS 账户。公司在公司共享服务账户中的 VPC 托管了一些应用程序。公司已将传输网关附加到共享服务账户中的 VPC。公司正在开发一种新功能，并创建了一个开发环境，该环境需要访问共享服务账户中的应用程序。公司打算在开发账户中频繁地删除和重新创建资源。公司还希望给开发团队提供根据需要重新创建团队与共享服务账户的连接的能力。哪种解决方案能满足这些要求？

- A. 在开发账户中创建一个传输网关。向共享服务账户创建一个传输网关对等请求。配置共享服务传输网关自动接受对等连接。
- B. 在共享服务账户中打开传输网关的自动接受功能。使用 AWS 资源访问管理器(AWS RAM)在共享服务账户中共享传输网关资源与开发账户。在开发账户中接受资源。在开发账户中创建一个传输网关附加。
- C. 在共享服务账户中打开传输网关的自动接受功能。创建一个 VPC 终端节点。使用终端节点策略为开发账户授予 VPC 终端节点的权限。配置终端节点服务自动接受连接请求。
- D. 创建一个 Amazon EventBridge 规则来调用 AWS Lambda 函数，当开发账户进行附加请求时接受传输网关附加。使用 AWS 网络管理器在共享服务账户中共享传输网关与开发账户。在开发账户中接受传输网关。

---

### 单选题 230/306

一家公司使用 AWS Organizations 管理其 AWS 账户。解决方案架构师必须设计一个解决方案，其中只有管理员角色被允许使用 IAM 操作。然而，解决方案架构师无法访问公司内所有的 AWS 账户。哪种解决方案以最小的运营开销满足这些要求？

- A. 创建一个 SCP，适用于所有 AWS 账户，仅允许管理员角色使用 IAM 操作。将 SCP 应用于根 OU。
- B. 配置 AWS CloudTrail 以调用 AWS Lambda 函数，用于每个与 IAM 操作相关的事件。配置函数以在触发操作的用户不是管理员时拒绝该操作。
- C. 创建一个 SCP，适用于所有 AWS 账户，否认所有用户使用 IAM 操作，除了那些具有管理员角色的用户。将 SCP 应用于根 OU。
- D. 设置一个 IAM 权限边界，允许 IAM 操作。将权限边界附加到所有 AWS 账户的每个管理员角色。

---

### 单选题 231/306

一家公司在其本地数据中心运行许多服务。该数据中心使用 AWS Direct Connect (DX) 和 IPSec VPN 连接到 AWS。服务数据是敏感的，并且连接不能通过互联网。公司希望扩展到新的市场细分，并开始向使用 AWS 的其他公司提供其服务。哪种解决方案能满足这些要求？

- A. 创建一个接受 TCP 流量的 VPC 终端节点服务，将其托管在网络负载均衡器后面，并通过 DX 使其服务可用。
- B. 创建一个接受 HTTP 或 HTTPS 流量的 VPC 终端节点服务，将其托管在应用程序负载均衡器后面，并通过 DX 使其服务可用。
- C. 附加一个互联网网关到 VPC，并确保网络访问控制和安全组规则允许相关的入站和出站流量。
- D. 附加一个 NAT 网关到 VPC，并确保网络访问控制和安全组规则允许相关的入站和出站流量。

---

### 单选题 232/306

一家公司需要为在单一 AWS 区域运行的关键应用程序实施灾难恢复。应用程序的用户与托管在应用程序负载均衡器(ALB)背后的 Amazon EC2 实例上的 Web 前端交互。应用程序写入 Amazon RDS for MySQL 数据库实例。应用程序还输出处理过的文档，这些文档存储在 Amazon S3 存储桶中。公司的财务团队直接查询数据库运行报告。在高峰时段，这些查询消耗资源并对应用程序性能产生负面影响。解决方案架构师必须设计一个解决方案，以在灾难期间提供弹性。该解决方案必须最小化数据丢失，并必须解决财务团队查询所导致的性能问题。哪种解决方案能满足这些要求？

- A. 将数据库迁移到 Amazon DynamoDB 并使用 DynamoDB 全球表。指示财务团队查询单独区域中的全球表。创建 AWS Lambda 函数定期同步原始 S3 存储桶的内容到单独区域中的新 S3 存储桶。在单独区域中启动 EC2 实例并创建 ALB。配置应用程序指向新的 S3 存储桶。
- B. 在单独区域中启动附加的 EC2 实例以托管应用程序。将附加实例添加到单独区域中的现有 ALB，创建 RDS 数据库实例的读取副本。指示财务团队对读取副本运行查询。使用 S3 跨区域复制(CRR)从原始 S3 存储桶复制到单独区域中的新 S3 存储桶。在灾难期间，将读取副本提升为独立的数据库实例。配置应用程序指向新的 S3 存储桶和新提升的读取副本。
- C. 在单独区域中创建 RDS 数据库实例的读取副本。指示财务团队对读取副本运行查询。创建托管应用程序前端的 EC2 实例的 AMI。将 AMI 复制到单独区域。使用 S3 跨区域复制(CRR)从原始 S3 存储桶复制到单独区域中的新 S3 存储桶。在灾难期间，将读取副本提升为独立的数据库实例。从 AMI 启动 EC2 实例并创建 ALB 以向最终用户呈现应用程序。配置应用程序指向新的 S3 存储桶。
- D. 创建 RDS 数据库实例的每小时快照。将快照复制到单独区域。在现有 RDS 数据库前添加 Amazon ElastiCache 集群。创建托管应用程序前端的 EC2 实例的 AMI。将 AMI 复制到单独区域。使用 S3 跨区域复制(CRR)从原始 S3 存储桶复制到单独区域中的新 S3 存储桶。在灾难期间，从最新的 RDS 快照恢复数据库。从 AMI 启动 EC2 实例并创建 ALB 以向最终用户呈现应用程序。配置应用程序指向新的 S3 存储桶。

---

### 单选题 233/306

一家公司需要将其网站从本地数据中心迁移到 AWS。该网站由负载均衡器、在 Linux 操作系统上运行的内容管理系统(CMS)和 MySQL 数据库组成。CMS 需要持久的 NFS 兼容存储用于文件系统。新的 AWS 解决方案必须能够在不可预测的流量增加时，从 2 个 Amazon EC2 实例扩展到 30 个 EC2 实例。新解决方案还必须不需要对网站进行更改，并且必须防止数据丢失。哪种解决方案能满足这些要求？

- A. 创建一个 Amazon Elastic File System (Amazon EFS) 文件系统。使用应用程序负载均衡器和自动扩展组将 CMS 部署到 AWS Elastic Beanstalk。使用 .ebextensions 将 EFS 文件系统挂载到 EC2 实例。创建一个与 Elastic Beanstalk 环境分开的 Amazon Aurora MySQL 数据库。
- B. 创建一个 Amazon Elastic Block Store (Amazon EBS) Multi-Attach 卷。使用网络负载均衡器和自动扩展组将 CMS 部署到 AWS Elastic Beanstalk。使用 .ebextensions 将 EBS 卷挂载到 EC2 实例。在 Elastic Beanstalk 环境中创建一个用于 MySQL 的 Amazon RDS 数据库。
- C. 创建一个 Amazon Elastic File System (Amazon EFS) 文件系统。创建启动模板和自动扩展组以启动 EC2 实例来支持 CMS。创建一个网络负载均衡器来分配流量。创建一个 Amazon Aurora MySQL 数据库。使用 EC2 自动扩展缩减生命周期钩子将 EFS 文件系统挂载到 EC2 实例。
- D. 创建一个 Amazon Elastic Block Store (Amazon EBS) Multi-Attach 卷。创建启动模板和自动扩展组以启动 EC2 实例来支持 CMS。创建应用程序负载均衡器来分配流量。创建一个支持 MySQL 数据库的 Amazon ElastiCache for Redis 集群。使用 EC2 用户数据将 EBS 卷附加到 EC2 实例。

---

### 单选题 234/306

一家公司正在使用 GitHub Actions 运行一个 CI/CD 流水线，该流水线访问 AWS 上的资源。该公司有一个 IAM 用户，该用户在流水线中使用密钥对来认证到 AWS。一个现有的 IAM 角色附加了一个策略，授予部署资源所需的权限。公司的安全团队实施了一个新的要求，流水线不能再使用长期密钥。解决方案架构师必须用一个短期解决方案替换长期密钥。哪种解决方案能满足这些要求且运营开销最小？

- A. 在 AWS 身份和访问管理(IAM)中创建一个 IAM SAML 2.0 身份提供者(IdP)。创建一个新的 IAM 角色，该角色的信任策略允许 sts:AssumeRole API 调用。将现有的 IAM 策略附加到这个新 IAM 角色。更新 GitHub 以使用 SAML 认证流水线。
- B. 在 AWS 身份和访问管理(IAM)中创建一个 IAM OpenID Connect(OIDC)身份提供者(IdP)。创建一个新的 IAM 角色，该角色的信任策略允许来自 GitHub OIDC IdP 的 sts:AssumeRoleWithWebIdentity API 调用。更新 GitHub 以假设角色用于流水线。
- C. 创建一个 Amazon Cognito 身份池。将认证提供者配置为使用 GitHub。创建一个新的 IAM 角色，该角色的信任策略允许来自 GitHub 认证提供者的 sts:AssumeRoleWithWebIdentity API 调用。将流水线配置为使用 Cognito 作为其认证提供者。
- D. 为 AWS 私有证书颁发机构创建一个信任锚。生成一个客户端证书，与 AWS IAM Roles Anywhere 一起使用。创建一个新的 IAM 角色，该角色的信任策略允许 sts:AssumeRole API 调用。将现有的 IAM 策略附加到这个新 IAM 角色。将流水线配置为使用凭证帮助工具，并引用客户端证书公钥以假设新 IAM 角色。

---

### 单选题 235/306

一家公司有一个在本地运行的应用程序，该程序分析并存储图像数据。该应用程序每天接收数百万张新的图像文件。文件平均大小为 1MB。文件以 1GB 的批次进行分析。当应用程序分析一个批次时，应用程序会将图像压缩在一起。然后，应用程序将图像作为单个文件在本地 NFS 服务器上进行归档，以进行长期存储。该公司在本地拥有 Microsoft Hyper-V 环境并且有可用的计算能力。公司没有存储容量，想要在 AWS 上归档图像。公司需要在请求后 1 周内检索归档数据的能力。该公司在本地数据中心和 AWS 之间有一条 10Gbps 的 AWS Direct Connect 连接。公司需要设置带宽限制，并安排在非工作时间将归档图像复制到 AWS。哪种解决方案最具成本效益？

- A. 在新的基于 GPU 的 Amazon EC2 实例上部署 AWS DataSync 代理。配置 DataSync 代理将 NFS 本地服务器上的文件批次复制到 Amazon S3 Glacier 即时检索。成功复制后，从本地存储中删除数据。
- B. 在本地的 Hyper-V 环境中作为 Hyper-V VM 部署 AWS DataSync 代理。配置 DataSync 代理将 NFS 本地服务器上的文件批次复制到 Amazon S3 Glacier Deep Archive。成功复制后，从本地存储中删除数据。
- C. 在新的通用 Amazon EC2 实例上部署 AWS DataSync 代理。配置 DataSync 代理将 NFS 本地服务器上的文件批次复制到 Amazon S3 标准存储。成功复制后，从本地存储中删除数据。创建一个 S3 生命周期规则，在 1 天后将对象从 S3 标准过渡到 S3 Glacier Deep Archive。
- D. 在本地 Hyper-V 环境中部署 AWS Storage Gateway 磁带网关。将磁带网关连接到 AWS。使用自动磁带创建。指定 Amazon S3 Glacier Deep Archive 池。复制完图像批次后弹出磁带。

---

### 单选题 236/306

解决方案架构师必须为一队云工程师提供一种安全的方法，使用 AWS CLI 上传对象到 Amazon S3 存储桶。每位云工程师都有一个 IAM 用户、IAM 访问密钥和虚拟多因素认证(MFA)设备。云工程师的 IAM 用户位于一个名为 S3-access 的组中。云工程师必须使用 MFA 执行在 Amazon S3 中的任何操作。哪种解决方案能满足这些要求？

- A. 附加一个策略到 S3 存储桶，在 IAM 用户对存储桶执行操作时提示输入 MFA 代码。使用 IAM 访问密钥和 AWS CLI 调用 Amazon S3。
- B. 更新 S3-access 组的信任策略，要求主体在承担组时使用 MFA。使用 IAM 访问密钥和 AWS CLI 调用 Amazon S3。
- C. 附加一个策略到 S3-access 组，除非存在 MFA，否则拒绝所有 S3 操作。使用 IAM 访问密钥和 AWS CLI 调用 Amazon S3。
- D. 附加一个策略到 S3-access 组，除非存在 MFA，否则拒绝所有 S3 操作。从 AWS 安全令牌服务(AWS STS)请求临时凭证。将临时凭证附加到一个配置文件中，当用户在 Amazon S3 中执行操作时，Amazon S3 将引用该配置文件。

---

### 单选题 237/306

一家公司在 Amazon Elastic File System (Amazon EFS) 文件系统中存储和管理文档。该文件系统使用 AWS Key Management Service (AWS KMS) 密钥进行加密。文件系统挂载到一个运行专有软件的 Amazon EC2 实例上。公司已为文件系统启用了自动备份。自动备份使用 AWS Backup 默认备份计划。解决方案架构师必须确保在 100 分钟的恢复点目标(RPO)内能够恢复已删除的文档。哪种解决方案能满足这些要求？

- A. 创建一个新的 IAM 角色。创建一个新的备份计划。使用新的 IAM 角色创建备份。更新 KMS 密钥策略，允许新的 IAM 角色使用该密钥。为文件系统实现每小时备份计划。
- B. 创建一个新的备份计划。更新 KMS 密钥策略，允许 AWSServiceRoleForBackup IAM 角色使用该密钥。实现每 30 分钟运行文件系统备份的自定义 cron 表达式。
- C. 创建一个新的 IAM 角色。使用现有的备份计划。更新 KMS 密钥策略，允许新的 IAM 角色使用该密钥。启用持续备份以实现点对点恢复。
- D. 使用现有的备份计划。更新 KMS 密钥策略，允许 AWSServiceRoleForBackup IAM 角色使用该密钥。为文件系统启用跨区域复制。

---

### 单选题 238/306

一家公司正在为制造应用程序设计 AWS 环境。该应用程序已经获得客户成功，应用程序的用户基础已经增加。公司已经通过 1Gbps AWS Direct Connect 连接将 AWS 环境连接到公司的本地数据中心。公司已为连接配置了 BGP。公司必须更新现有的网络连接解决方案，以确保解决方案具有高可用性、容错性和安全性。哪种解决方案将以最具成本效益的方式满足这些要求？

- A. 添加一个动态的私有 IP AWS 站点到站点 VPN 作为 Direct Connect 连接的次要路径，以确保数据传输安全，并为 Direct Connect 连接提供弹性。在 Direct Connect 连接内配置 MACsec 加密流量。
- B. 在公司的本地数据中心和 AWS 之间提供另一个 Direct Connect 连接，以增加传输速度并为 Direct Connect 连接提供弹性。在 Direct Connect 连接内配置 MACsec 加密流量。
- C. 配置多个私有 VIF。在本地数据中心和 AWS 之间跨 VIF 进行数据负载均衡，以提供弹性。
- D. 添加一个静态的 AWS 站点到站点 VPN 作为 Direct Connect 连接的次要路径，以确保数据传输安全，并为 Direct Connect 连接提供弹性。

---

### 单选题 239/306

一家拥有多个业务单元的公司正在使用 AWS Organizations，所有功能都已启用。公司实施了一个账户结构，其中每个业务单元都有自己的 AWS 账户。每个 AWS 账户的管理员都需要使用 Amazon Athena 查看其账户的详细成本和使用情况数据。每个业务单元只能访问自己的成本和使用情况数据。管理设置 AWS 成本和使用情况报告能力的 IAM 策略已经就位。一个包含组织所有数据的中央成本和使用情况报告已经在 Amazon S3 存储桶中可用。哪种解决方案将以最小的运营复杂性满足这些要求？

- A. 在组织的管理账户中，使用 AWS 资源访问管理器（AWS RAM）与每个成员账户共享成本和使用情况报告数据。
- B. 在组织的管理账户中，配置 S3 事件以在包含中央成本和使用情况报告的 S3 存储桶中每次有新文件到达时调用 AWS Lambda 函数。配置 Lambda 函数提取每个成员账户的数据，并将数据放置在 Amazon S3 下的单独前缀下。修改 S3 存储桶策略，以允许每个成员账户访问自己的前缀。
- C. 在每个成员账户中，访问 AWS 成本资源管理器。创建一个包含账户相关成本信息的新报告。在成本资源管理器中保存报告。提供账户管理员用于访问保存的报告的说明。
- D. 在每个成员账户中，创建一个新的 S3 存储桶以存储成本和使用情况报告数据。设置成本和使用情况报告将数据传递到新的 S3 存储桶。

---

### 单选题 240/306

一家公司使用 AWS CloudFormation 在其 AWS 成员账户中创建所有新基础设施。资源很少更改，并且已正确调整以适应预期的负载。每月的 AWS 账单是一致的。偶尔，开发人员会为测试创建一个新资源，并在测试完成后忘记删除资源。这些测试中的大多数持续几天，然后资源就不再需要了。公司希望自动化查找未使用资源的过程。解决方案架构师需要设计一个解决方案，确定 AWS 账单中的成本是否在增加。该解决方案必须帮助识别导致成本增加的资源，并自动通知公司的操作团队。哪种解决方案将满足这些要求？

- A. 打开账单警报。使用 AWS 成本资源管理器确定上个月的成本。为总估算费用创建 Amazon CloudWatch 警报。指定高于成本资源管理器确定的成本的成本阈值。添加通知，如果警报阈值被突破，则通知操作团队。
- B. 打开账单警报。使用 AWS 成本资源管理器确定过去 3 个月的平均每月成本。为总估算费用创建 Amazon CloudWatch 警报。指定高于成本资源管理器确定的成本的成本阈值。添加通知，如果警报阈值被突破，则通知操作团队。
- C. 使用 AWS 成本异常检测创建具有链接账户监视类型的成本监视器。创建一个订阅，将每日 AWS 成本摘要发送给操作团队。指定成本差异的阈值。
- D. 使用 AWS 成本异常检测创建具有 AWS 服务监视类型的成本监视器。创建一个订阅，将每日 AWS 成本摘要发送给操作团队。指定成本差异的阈值。

---

### 单选题 241/306

一家公司使用 AWS Organizations 管理其开发环境。公司的每个开发团队都有自己的 AWS 账户。每个账户都有一个单一的 VPC 和不重叠的 CIDR 块。公司在共享服务账户中有一个 Amazon Aurora DB 集群。所有开发团队都需要使用 DB 集群中的实时数据。哪种解决方案将以最小的运营开销提供对 DB 集群所需的连接？

- A. 为 DB 集群创建 AWS 资源访问管理器（AWS RAM）资源共享。将 DB 集群与所有开发账户共享。
- B. 在共享服务账户中创建传输网关。为传输网关创建 AWS 资源访问管理器（AWS RAM）资源共享。将传输网关与所有开发账户共享。指示开发人员接受资源共享并配置网络。
- C. 建指向 DB 集群 IP 地址的应用程序负载均衡器（ALB）。创建使用 ALB 的 AWS PrivateLink 终端节点服务。添加权限，允许每个开发账户连接到终端节点服务。
- D. 在共享服务账户中创建 AWS 站点到站点 VPN 连接。配置网络。在每个开发账户中使用 AWS Marketplace VPN 软件连接到站点到站点 VPN 连接。

---

### 多选题 242/306

一家公司有多个业务线（LOB），它们都汇总到母公司。公司已要求其解决方案架构师开发满足以下要求的解决方案：

- 为所有 LOB 使用的 AWS 账户生成单一 AWS 发票。
- 发票上应该详细列出每个 LOB 账户的费用。
- 提供根据公司治理政策限制 LOB 账户中服务和功能的能力。
- 每个 LOB 账户应委派完全的管理员权限，无论治理政策如何。

解决方案架构师应该采取哪种组合的步骤来满足这些要求？（选择两个）

- A. 使用 AWS Organizations 在母账户中为每个 LOB 创建一个组织。然后邀请每个 LOB 账户加入相应的组织。
- B. 使用 AWS Organizations 在母账户中创建一个单一组织。然后邀请每个 LOB 的 AWS 账户加入该组织。
- C. 实施服务配额，定义允许的服务和功能，并将配额应用到每个 LOB，视情况而定。
- D. 创建一个 SCP，允许仅使用批准的服务和功能，然后将策略应用到 LOB 账户。
- E. 在母账户的计费控制台中启用合并计费，并将 LOB 账户链接起来。

---

### 多选题 243/306

一家公司想要将 Amazon Aurora MySQL DB 集群从一个现有的 AWS 账户迁移到同一 AWS Region 中的新 AWS 账户。两个账户都是 AWS Organizations 组织中的成员。公司必须最小化数据库服务中断，然后公司执行 DNS 切换到新数据库。哪种迁移策略将满足此要求？（选择两个）

- A. 拍摄现有 Aurora 数据库的快照。与新 AWS 账户共享快照。在新账户中从快照创建 Aurora DB 集群。
- B. 在新 AWS 账户中创建 Aurora DB 集群。使用 AWS 数据库迁移服务（AWS DMS）在两个 Aurora DB 集群之间迁移数据。
- C. 使用 AWS 备份共享来自现有 AWS 账户的 Aurora 数据库备份到新 AWS 账户。在新账户中从快照创建 Aurora DB 集群。
- D. 在新 AWS 账户中创建 Aurora DB 集群。使用 AWS 应用程序迁移服务在两个 Aurora DB 集群之间迁移数据。

---

### 单选题 244/306

一家公司在手动创建的 VPC 中运行其解决方案。公司使用 AWS CloudFormation 来配置基础设施的其他部分。根据新要求，公司必须以自动化的方式管理所有基础设施。为了以最小的努力满足这个新要求，公司应该怎么办？

- A. 创建一个新的 AWS Cloud Development Kit（AWS CDK）堆栈，严格配置现有的 VPC 资源和配置。使用 AWS CDK 导入 VPC 到堆栈并管理 VPC。
- B. 创建一个 CloudFormation 堆栈集，创建 VPC。使用堆栈集将 VPC 导入到堆栈。
- C. 创建一个新的 CloudFormation 模板，严格配置现有的 VPC 资源和配置。从 CloudFormation 控制台创建一个新的堆栈，通过导入现有资源。
- D. 创建一个新的 CloudFormation 模板，创建 VPC。使用 AWS Serverless Application Model（AWS SAM）CLI 导入 VPC。

---

### 单选题 245/306

一家公司已实施了一项新的安全要求。根据新要求，公司必须扫描公司 VPC 中所有企业 AWS 实例的所有流量，以检查是否违反公司的安全政策。由于这些扫描，公司可以阻止访问特定 IP 地址。为了满足新要求，公司在私有子网中部署了一组 Amazon EC2 实例，作为透明代理服务器。公司在这些 EC2 实例上安装了经过批准的代理服务器软件。公司修改了所有子网的路由表，使用带有代理软件的相应 EC2 实例作为默认路由。公司还创建了符合安全政策的安全组，并将这些安全组分配给 EC2 实例。尽管进行了这些配置，EC2 实例在私有子网中的流量没有正确转发到互联网。解决方案架构师应该做些什么来解决这个问题？

- A. 在运行代理软件的 EC2 实例上禁用源/目的检查。
- B. 向分配给代理 EC2 实例的安全组添加一条规则，以允许具有此安全组的所有实例之间的所有流量。将此安全组分配给 VPC 中的所有 EC2 实例。
- C. 更改 VPC 的 DHCP 选项集。将 DNS 服务器选项设置为指向代理 EC2 实例的地址。
- D. 为每个代理 EC2 实例分配一个额外的弹性网络接口。确保其中一个网络接口有一条路由到私有子网。确保另一个网络接口有一条路由到互联网。

---

### 单选题 246/306

一家公司提供了一个在单个共享 VPC 中托管的集中式 Amazon EC2 应用程序。集中式应用程序必须能够从其他业务单元的 VPC 中运行的客户端应用程序访问。集中式应用程序前端配置了网络负载均衡器（NLB）以实现可扩展性。最多需要将 10 个业务单元 VPC 连接到共享 VPC。一些业务单元 VPC 的 CIDR 块与共享 VPC 重叠，有些彼此重叠。从授权的业务单元 VPC 到共享 VPC 中的集中式应用程序的网络连接应该是被允许的。解决方案架构师应该使用哪种网络配置来提供从业务单元 VPC 中的客户端应用程序到共享 VPC 中的集中式应用程序的连接？

- A. 创建一个 AWS 传输网关。将共享 VPC 和授权的业务单元 VPC 连接到传输网关。创建一个单一的传输网关路由表，并将所有连接的 VPC 与之关联。允许从连接自动传播路由到路由表。配置 VPC 路由表将流量发送到传输网关。
- B. 使用集中式应用程序 NLB 创建 VPC 终端节点服务，并启用要求终端节点接受的选项。在每个业务单元 VPC 中使用终端节点服务的服务名称创建 VPC 终端节点。从终端节点服务控制台接受授权的终端节点请求。
- C. 从每个业务单元 VPC 到共享 VPC 创建 VPC 对等连接。从共享 VPC 控制台接受 VPC 对等连接。配置 VPC 路由表将流量发送到 VPC 对等连接。
- D. 为共享 VPC 配置一个虚拟专用网关，并为每个授权的业务单元 VPC 创建客户网关。从业务单元 VPC 到共享 VPC 建立站点到站点 VPN 连接。配置 VPC 路由表将流量发送到 VPN 连接。

---

### 单选题 247/306

一家公司在 eu-north-1 Region 的 Amazon EC2 上运行一个高度可用的数据收集应用程序。该应用程序从最终用户设备收集数据，并将记录写入 Amazon Kinesis 数据流和一组处理记录的 AWS Lambda 函数。公司将记录处理的输出持久化到 eu-north-1 的 Amazon S3 存储桶中。公司使用 S3 存储桶中的数据作为 Amazon Athena 的数据源。公司希望增加其全球影响力。解决方案架构师必须在 sa-east-1 和 ap-northeast-1 Region 中启动数据收集能力。解决方案架构师在两个新 Region 中部署了应用程序、Kinesis 数据流和 Lambda 函数。解决方案架构师将 S3 存储桶保留在 eu-north-1，以满足将数据分析集中化的要求。在测试新设置期间，解决方案架构师注意到来自新 Region 的数据到达 S3 存储桶有明显的延迟。哪种解决方案将最大程度地改善这种延迟时间？

- A. 在两个新 Region 中的每一个中设置 Lambda 函数以在 VPC 中运行。在该 VPC 中设置 S3 网关端点。
- B. 在 eu-north-1 的 S3 存储桶上打开 S3 传输加速。更改应用程序，以便在应用程序上传数据到 S3 存储桶时使用新的 S3 加速端点。
- C. 在两个新 Region 中的每一个中创建一个 S3 存储桶。将每个新 Region 中的应用程序设置为上传到其各自的 S3 存储桶。设置 S3 跨 Region 复制以将数据复制到 eu-north-1 的 S3 存储桶。
- D. 增加 Lambda 函数的内存需求，以确保它们有多个核心可用。使用 Lambda 从应用程序上传数据到 Amazon S3 时使用多部分上传功能。

---

### 单选题 248/306

一家医疗公司在一组 Amazon EC2 实例上运行 REST API。EC2 实例在自动扩展组中运行，位于应用程序负载均衡器（ALB）后面。ALB 在三个公共子网中运行，而 EC2 实例在三个私有子网中运行。公司部署了一个 Amazon CloudFront 分发，ALB 是唯一的起源。解决方案架构师应推荐哪种解决方案来增强起源安全性？

- A. 在 AWS Secrets Manager 中存储一个随机字符串。创建一个用于自动秘密轮换的 AWS Lambda 函数。配置 CloudFront 以将随机字符串作为自定义 HTTP 头注入到起源请求中。创建一个带有字符串匹配规则的 AWS WAF Web ACL 规则，用于自定义头。将 Web ACL 与 ALB 关联。
- B. 创建一个带有 CloudFront 服务 IP 地址范围的 IP 匹配条件的 AWS WAF Web ACL 规则。将 Web ACL 与 ALB 关联。将 ALB 移动到三个私有子网中。
- C. 在 AWS Systems Manager 参数存储中存储一个随机字符串。配置参数存储以自动轮换字符串。配置 CloudFront 以将随机字符串作为自定义 HTTP 头注入到起源请求中。检查自定义 HTTP 头的值，并在 ALB 中阻止访问。
- D. 配置 AWS Shield Advanced。创建一个安全组策略，允许来自 CloudFront 服务 IP 地址范围的连接。将策略添加到 AWS Shield Advanced，并将策略附加到 ALB。

---

### 单选题 249/306

一家公司计划将本地数据中心迁移到 AWS。公司目前在基于 Linux 的 VMware 虚拟机上托管数据中心。解决方案架构师需要收集有关 VM 之间网络依赖关系的信息。这些信息必须以图表形式呈现，详细列出主机 IP 地址、主机名和网络连接信息。哪种解决方案将满足这些要求？

- A. 使用 AWS 应用程序发现服务。选择 AWS 迁移中心主 AWS 区域。在本地服务器上安装 AWS 应用程序发现代理，进行数据收集。授予应用程序发现服务使用迁移中心网络图表的权限。
- B. 使用 AWS 应用程序发现服务的无代理收集器进行服务器数据收集。从 AWS 迁移中心导出网络图表为.png 格式。
- C. 在本地服务器上安装 AWS 应用程序迁移服务代理，进行数据收集。使用 AWS 迁移中心的数据在 AWS 上生成网络图表。
- D. 在本地服务器上安装 AWS 应用程序迁移服务代理，进行数据收集。将 AWS 迁移中心的数据导出为.csv 格式，导入到 Amazon CloudWatch 仪表板以生成网络图表。

---

### 单选题 250/306

一家公司有一个带有 Web 前端的应用程序。该应用程序在公司的本地数据中心运行，需要访问文件存储来存储关键数据。该应用程序在三台 Linux 虚拟机上运行以实现冗余。架构包括一个基于 HTTP 请求路由的负载均衡器。公司需要尽快将应用程序迁移到 AWS。AWS 上的架构必须是高可用的。哪种解决方案将以最少的架构更改满足这些要求？

- A. 将应用程序迁移到使用三个可用区中的 Fargate 启动类型的 Amazon Elastic Container Service（Amazon ECS）容器中。使用 Amazon S3 为所有三个容器提供文件存储。使用 Network Load Balancer 将流量定向到容器。
- B. 将应用程序迁移到三个可用区中的 Amazon EC2 实例。使用 Amazon Elastic File System（Amazon EFS）进行文件存储。在所有三个 EC2 实例上挂载文件存储。使用 Application Load Balancer 将流量定向到 EC2 实例。
- C. 将应用程序迁移到使用三个可用区中的 Fargate 启动类型的 Amazon Elastic Kubernetes Service（Amazon EKS）容器中。使用 Amazon FSx for Lustre 为所有三个容器提供文件存储。使用 Network Load Balancer 将流量定向到容器。
- D. 将应用程序迁移到三个 AWS Region 中的 Amazon EC2 实例。使用 Amazon Elastic Block Store（Amazon EBS）进行文件存储。为所有三个 EC2 实例启用跨区域复制（CRR）。使用 Application Load Balancer 将流量定向到 EC2 实例。
...
- A. 将ALB配置为通过与IdP集成来执行身份验证和授权。仅允许经过身份验证的用户访问后端服务。
- B. 修改CloudFront配置以使用签名URL。实施允许任何请求访问后端服务的宽松签名策略。
- C. 创建一个AWS WAF Web ACL，在ALB级别过滤未经身份验证的请求。仅允许经过身份验证的流量到达后端服务。
- D. 启用AWS CloudTrail记录到ALB的所有请求。创建一个AWS Lambda函数来分析日志，并阻止任何来自未经身份验证的用户的请求。
...
- A. 创建一个新的AWS账户，该账户只能通过安全团队的角色进行访问。在新账户中创建一个S3存储桶。启用S3版本控制和S3对象锁定。设置1年的默认保留期。从现有的S3存储桶到新的S3存储桶设置复制。创建一个S3批量复制作业以复制所有现有数据。
- B. 使用s3-bucket-versioning-enabled AWS Config托管规则。配置一个自动补救操作，使用AWS Lambda函数启用S3版本控制和MFA删除功能在不符合规定的资源上。添加一个S3生命周期规则，在1年后删除对象。
- C. 明确拒绝除AWS服务目录启动约束角色外的所有用户和角色创建存储桶。定义一个服务目录产品，用于创建S3存储桶，以强制启用S3版本控制和MFA删除。授权用户在需要创建S3存储桶时启动产品。
- D. 为账户和AWS Region启用Amazon GuardDuty，并启用S3保护功能。添加一个S3生命周期规则，在1年后删除对象。
...
- A. 创建一个可以验证序列号的AWS Lambda函数。创建一个AWS IoT Core预置模板。在参数部分中包含SerialNumber参数。将Lambda函数添加为预置的钩子。在生产过程中，调用RegisterThing API操作并指定模板和参数。
- B. 创建一个可以验证序列号的AWS Step Functions状态机。创建一个AWS IoT Core预置模板。在参数部分中包含SerialNumber参数。指定Step Functions状态机来验证参数。在安装期间调用StartThingRegistrationTask API操作。
- C. 创建一个可以验证序列号的AWS Lambda函数。创建一个AWS IoT Core预置模板。在参数部分中包含SerialNumber参数。将Lambda函数添加为预置的钩子。注册CA到AWS IoT Core，指定预置模板，并设置allow-auto-registration参数。
- D. 创建一个AWS IoT Core预置模板。在参数部分中包含SerialNumber参数。在模板中包含参数验证。使用CA为每个设备预置索赔证书和私钥。授予AWS IoT Core服务权限，在预置期间更新AWS IoT设备。
...
- A. 在所有相关账户中启用新Region。在创建堆栈集时指定CAPABILITY_NAMED_IAM能力。
- B. 使用服务配额控制台请求每个新Region中CloudFormation堆栈数量的配额增加。在创建堆栈集时指定CAPABILITY_IAM能力。
- C. 在创建堆栈集时指定CAPABILITY_NAMED_IAM能力和SELF_MANAGED权限模型。
- D. 在创建堆栈集时指定一个管理角色ARN和CAPABILITY_IAM能力。
...
- A. 为OU中每个账户中的所有实例配置AWS Systems Manager。使用Systems Manager Automation运行簿来防止公共IP地址附加到实例。
- B. 实施AWS Control Tower主动控制，检查OU账户中的实例是否具有公共IP地址。将AssociatePublicIpAddress属性设置为False。将主动控制附加到OU。
- C. 创建一个SCP，防止启动具有公共IP地址的实例。另外，配置SCP以防止将公共IP地址附加到现有实例。将SCP附加到OU。
- D. 创建一个AWS Config自定义规则，检测具有公共IP地址的实例。配置一个使用AWS Lambda函数的修复操作，以从实例中分离公共IP地址。
...
- A. 使用单独的启动模板将EKS控制平面部署到与工作负载节点组不同的第二个集群中。
- B. 更新工作负载节点组。使用较少的节点组和较大的实例。
- C. 配置Kubernetes Cluster Autoscaler，以确保工作负载节点组的计算容量保持供应不足。
- D. 配置工作负载使用基于可用区的拓扑扩散约束。
...
- A. 在单独的Region中设置第二个ECS集群和ECS服务。创建一个AWS Lambda函数以执行以下操作：拍摄RDS DB实例的快照，将快照复制到单独的Region，从快照创建新的RDS DB实例，并更新Route 53以将流量路由到第二个ECS集群。更新EventBridge规则以添加一个目标，该目标将调用Lambda函数。
- B. 创建一个AWS Lambda函数，在单独的Region中创建第二个ECS集群和ECS服务。配置Lambda函数以执行以下操作：拍摄RDS DB实例的快照，将快照复制到单独的Region，从快照创建新的RDS DB实例，并更新Route 53以将流量路由到第二个ECS集群。更新EventBridge规则以添加一个目标，该目标将调用Lambda函数。
- C. 在单独的Region中设置第二个ECS集群和Fargate上的ECS服务。在单独的Region中创建RDS DB实例的跨Region读取副本。创建一个AWS Lambda函数，以提升读取副本为primary数据库。配置Lambda函数以更新Route 53，将流量路由到第二个ECS集群。更新EventBridge规则以添加一个目标，该目标将调用Lambda函数。
- D. 在单独的Region中设置第二个ECS集群和Fargate上的ECS服务。拍摄RDS DB实例的快照。将快照转换为Amazon DynamoDB全局表。创建一个AWS Lambda函数，以更新Route 53，将流量路由到第二个ECS集群。更新EventBridge规则以添加一个目标，该目标将调用Lambda函数。

---

### 单选题 258/306

一家公司已部署了Amazon Connect联系中心。联系中心的代理报告了大量计算机生成的呼叫。公司担心这些呼叫对成本和生产力的影响。公司希望有一个解决方案，允许代理将呼叫标记为垃圾电话，并且自动阻止这些号码将来再次到达代理。哪种解决方案最符合运营效率要求？

- A. 通过添加一个标记呼叫按钮自定义联系控制面板（CCP），该按钮将调用UpdateContactAttributes API的AWS Lambda函数。使用Amazon DynamoDB表存储垃圾电话号码。修改联系流程，查找更新的属性，并使用Lambda函数读取和写入DynamoDB表。
- B. 使用Amazon Connect规则的Contact Lens，查找垃圾电话。使用Amazon DynamoDB表存储垃圾电话号码。修改联系流程，查找规则，并调用AWS Lambda函数读取和写入DynamoDB表。
- C. 使用Amazon DynamoDB表存储垃圾电话号码。创建一个快速连接，代理可以从联系控制面板（CCP）转移垃圾电话。修改快速连接的联系流程，调用AWS Lambda函数写入DynamoDB表。
- D. 修改初始联系流程以请求呼叫者输入。如果代理没有收到输入，代理应该将呼叫者标记为垃圾电话。使用Amazon DynamoDB表存储垃圾电话号码。使用AWS Lambda函数读取和写入DynamoDB表。
...
- A. 使用Amazon WorkSpaces作为云桌面服务。设置到本地网络的VPN连接。创建AD连接器，并连接到本地Active Directory。使用AWS管理控制台激活Amazon WorkSpaces的MFA。
- B. 使用Amazon AppStream 2.0作为应用程序流服务。为员工配置桌面视图。设置到本地网络的VPN连接。在本地设置Active Directory Federation Services（AD FS）。通过VPN连接将VPC网络连接到AD FS。
- C. 使用Amazon WorkSpaces作为云桌面服务。设置到本地网络的VPN连接。创建AD连接器，并连接到本地Active Directory。为MFA配置RADIUS服务器。
- D. 使用Amazon AppStream 2.0作为应用程序流服务。在本地设置Active Directory Federation Services。配置MFA以授予用户访问AppStream 2.0的权限。
...
- A. 在Neptune VPC中创建三个公共子网，并通过网络网关路由流量。在三个新的公共子网中托管Lambda函数。
- B. 在Neptune VPC中创建三个私有子网，并通过网络网关路由互联网流量。在三个新的私有子网中托管Lambda函数。
- C. 在VPC之外托管Lambda函数。更新Neptune安全组以允许来自Lambda函数的IP范围的访问。
- D. 在VPC之外托管Lambda函数。为Neptune数据库创建VPC端点，并让Lambda函数通过VPC端点访问Neptune。
- E. 在Neptune VPC中创建三个私有子网。在三个新的隔离子网中托管Lambda函数。为DynamoDB创建VPC端点，并路由DynamoDB流量到VPC端点。
...
- A. 使用桥接网络模式创建任务。
- B. 使用awsvpc网络模式创建任务。
- C. 将安全组应用于Amazon EC2实例，并使用IAM角色让EC2实例访问其他资源。
- D. 将安全组应用于任务，并在启动时将IAM凭据传递到容器中以访问其他资源。
- E. 将安全组应用于任务，并使用IAM角色让任务访问其他资源。
...
- A. 直接将文件从移动软件上传到Amazon S3。使用S3事件通知在Amazon MQ队列中创建消息。
- B. 直接将文件从移动软件上传到Amazon S3。使用S3事件通知在Amazon Simple Queue Service（Amazon SQS）标准队列中创建消息。
- C. 调用AWS Lambda函数，在队列中有消息时执行图像处理。
- D. 调用S3批量操作作业，在队列中有消息时执行图像处理。
- E. 使用Amazon Simple Notification Service（Amazon SNS）在处理完成时向移动应用程序发送推送通知。
- F. 使用Amazon Simple Email Service（Amazon SES）在处理完成时向移动应用程序发送推送通知。
...
- A. 设置DynamoDB Accelerator（DAX）作为内存缓存。更新应用程序以使用DAX。为EC2实例创建Auto Scaling组。创建应用程序负载均衡器（ALB）。将Auto Scaling组设置为ALB的目标。更新Route 53记录，使用简单的路由策略，目标为ALB的DNS别名。在内容更新之前配置EC2实例的预定扩展。
- B. 设置Amazon ElastiCache for Redis。更新应用程序以使用ElastiCache。为EC2实例创建Auto Scaling组。创建Amazon CloudFront分发，并设置Auto Scaling组为分发的来源。更新Route 53记录，使用简单的路由策略，目标为CloudFront分发的DNS别名。在内容更新之前手动扩展EC2实例。
- C. 设置Amazon ElastiCache for Memcached。更新应用程序以使用ElastiCache。为EC2实例创建Auto Scaling组。创建应用程序负载均衡器（ALB）。将Auto Scaling组设置为ALB的目标。更新Route 53记录，使用简单的路由策略，目标为ALB的DNS别名。在内容更新之前配置应用程序的预定扩展。
- D. 设置DynamoDB Accelerator（DAX）作为内存缓存。更新应用程序以使用DAX。为EC2实例创建Auto Scaling组。创建Amazon CloudFront分发，并设置Auto Scaling组为分发的来源。更新Route 53记录，使用简单的路由策略，目标为CloudFront分发的DNS别名。在内容更新之前手动扩展EC2实例。

---

### 单选题 264/306

一家公司在AWS上托管了一个软件即服务（SaaS）解决方案。该解决方案有一个Amazon API Gateway API，提供HTTPS端点。API使用AWS Lambda函数进行计算。Lambda函数将数据存储在Amazon Aurora Serverless v1数据库中。公司使用AWS无服务器应用程序模型（AWS SAM）部署了该解决方案。该解决方案跨越了多个可用区，并且没有灾难恢复（DR）计划。解决方案架构师必须设计一个DR策略，可以在另一个AWS Region中恢复解决方案。该解决方案的恢复时间为5分钟，恢复点目标为1分钟。解决方案架构师应该做些什么来满足这些要求？

- A. 在目标Region创建Aurora Serverless v1数据库的读取副本。使用AWS SAM在目标Region部署解决方案。在灾难情况下，提升读取副本为主要副本。
- B. 将Aurora Serverless v1数据库更改为标准Aurora MySQL全局数据库，扩展到源Region和目标Region。使用AWS SAM在目标Region部署解决方案。
- C. 在目标Region创建一个具有多个写入实例的Aurora Serverless v1 DB集群。在目标Region启动解决方案。配置两个区域解决方案以主动-被动配置工作。
- D. 将Aurora Serverless v1数据库更改为标准Aurora MySQL全局数据库，扩展到源Region和目标Region。在目标Region启动解决方案。配置两个区域解决方案以主动-被动配置工作。
...
- A. 使用AWS账户根用户凭证登录AWS管理控制台，使用AWS Organizations发送到finance1@example.com的初始电子邮件中的64个字符密码。按要求设置IAM用户。
- B. 从管理账户，切换角色以假定新成员账户的账户ID的OrganizationAccountAccessRole角色。按要求设置IAM用户。
- C. 转到AWS管理控制台登录页面。选择“使用根账户凭证登录。”使用finance1@example.com的电子邮件地址和管理账户的根密码登录。按要求设置IAM用户。
- D. 转到AWS管理控制台登录页面。使用新成员账户的账户ID和Support1 IAM凭据登录。按要求设置IAM用户。
...
- A. 在AWS Organizations中为每个开发单元创建单独的OU。将创建的OU分配给公司的AWS账户。为DevelopmentUnit资源类型标签创建带有拒绝操作和StringNotEquals条件的单独SCP，匹配开发单元名称。将SCP分配给相应的OU。
- B. 在SAML联合身份验证期间传递DevelopmentUnit作为AWS安全令牌服务（AWS STS）会话标签的属性。更新开发人员所假定IAM角色的IAM策略，带有拒绝操作和StringNotEquals条件，用于DevelopmentUnit资源类型标签和aws:PrincipalTag/DevelopmentUnit。
- C. 在SAML联合身份验证期间传递DevelopmentUnit作为AWS安全令牌服务（AWS STS）会话标签的属性。创建一个带有允许操作和StringEquals条件的SCP，用于DevelopmentUnit资源类型标签和aws:PrincipalTag/DevelopmentUnit。将SCP分配给根OU。
- D. 为每个开发单元创建单独的IAM策略。对于每个IAM策略，添加带有允许操作和StringEquals条件的DevelopmentUnit资源类型标签和开发单元名称。在SAML联合身份验证期间，使用AWS安全令牌服务（AWS STS）分配IAM策略，并将开发单元名称与假定的IAM角色匹配。
...
- A. 创建一个Amazon CloudFront分发。创建一个CloudFront起源组。将每个额外Region的NLB添加到起源组中。为客户提供分发的边缘位置的IP地址范围。
- B. 创建一个AWS Global Accelerator标准加速器。为每个额外Region的NLB创建一个标准加速器端点。为客户提供Global Accelerator IP地址。
- C. 创建一个Amazon CloudFront分发。为每个额外Region的NLB创建自定义起源。为客户提供分发的边缘位置的IP地址范围。
- D. 创建一个AWS Global Accelerator自定义路由加速器。为自定义路由加速器创建一个监听器。添加每个额外Region的NLB的IP地址和端口。为客户提供Global Accelerator IP地址。
...
- A. 使用AWS Application Discovery Service，并在数据中心的每个虚拟机上部署数据收集代理。
- B. 在本地环境中配置所有服务器上的Amazon CloudWatch代理，并将指标发布到Amazon CloudWatch Logs。
- C. 使用AWS Application Discovery Service并启用现有虚拟化环境中的无代理发现。
- D. 在AWS管理控制台中启用AWS Application Discovery Service，并配置企业防火墙以允许通过VPN进行扫描。

---

### 单选题 269/306

一家公司提供图像存储服务，想要在AWS上部署一个面向客户的解决方案。数百万个人客户将使用该解决方案。该解决方案将接收大批量的大图像文件，调整文件大小，并将文件存储在Amazon S3存储桶中，最长存储6个月。解决方案必须能够处理显著的需求变化。解决方案还必须是在企业规模上可靠的，并且如果发生故障，具有重新运行处理作业的能力。哪种解决方案最具成本效益？

- A. 使用AWS Step Functions处理用户存储图像时发生的S3事件。运行一个AWS Lambda函数，将图像就地调整大小，并替换S3存储桶中的原始文件。创建一个S3生命周期到期策略，在6个月后使所有存储的图像过期。
- B. 使用Amazon EventBridge处理用户上传图像时发生的S3事件。运行一个AWS Lambda函数，将图像就地调整大小，并替换S3存储桶中的原始文件。创建一个S3生命周期到期策略，在6个月后使所有存储的图像过期。
- C. 使用S3事件通知调用AWS Lambda函数，当用户存储图像时。使用Lambda函数将图像就地调整大小，并在S3存储桶中存储原始文件。创建一个S3生命周期策略，在6个月后将所有存储的图像移动到S3标准不频繁访问（S3 Standard-IA）。
- D. 使用Amazon Simple Queue Service（Amazon SQS）处理用户存储图像时发生的S3事件。运行一个AWS Lambda函数，调整图像大小，并将调整大小的文件存储在S3存储桶中，该存储桶使用S3标准不频繁访问（S3 Standard-IA）。创建一个S3生命周期策略，在6个月后将所有存储的图像移动到S3 Glacier Deep Archive。
...
- A. 创建一个至少有两个私有子网、两个NAT网关和一个虚拟专用网关的VPC。
- B. 创建一个至少有两个公共子网、一个虚拟专用网关和一个互联网网关的VPC。
- C. 在本地网络和目标AWS网络之间创建AWS站点到站点VPN连接。
- D. 在本地网络和目标AWS网络之间创建AWS Direct Connect连接和Direct Connect网关。
- E. 在配置复制服务器时，选择使用私有IP地址进行数据复制的选项。
- F. 在配置目标服务器的启动设置时，选择确保恢复实例的私有IP地址与源服务器的私有IP地址相匹配的选项。
...
- A. 对EC2实例使用预测性扩展策略。将数据库托管在具有自动扩展读取副本的Amazon Aurora PostgreSQL Serverless v2 Multi-AZ DB实例上。创建一个AWS Step Functions状态机来运行并行AWS Lambda函数，在销售活动前预热数据库。创建一个Amazon EventBridge规则来调用状态机。
- B. 对EC2实例使用计划扩展策略。将数据库托管在具有自动扩展读取副本的Amazon RDS for PostgreSQL Multi-AZ DB实例上。创建一个Amazon EventBridge规则，调用AWS Lambda函数，在销售活动前创建一个更大的读取副本。切换到更大的读取副本。创建另一个EventBridge规则，调用另一个Lambda函数，在销售活动后缩小读取副本。
- C. 对EC2实例使用预测性扩展策略。将数据库托管在具有自动扩展读取副本的Amazon RDS for PostgreSQL MultiAZ DB实例上。创建一个AWS Step Functions状态机来运行并行AWS Lambda函数，在销售活动前预热数据库。创建一个Amazon EventBridge规则来调用状态机。
- D. 对EC2实例使用计划扩展策略。将数据库部署在Amazon Aurora PostgreSQL Multi-AZ DB集群上。创建一个Amazon EventBridge规则，调用AWS Lambda函数，在销售活动前创建一个更大的Aurora副本。切换到更大的Aurora副本。创建另一个EventBridge规则，调用另一个Lambda函数，在销售活动后缩小Aurora副本。
...
- A. 使用拒绝列表策略。
- B. 查看AWS IAM中的访问顾问，以确定最近使用的服务。
- C. 查看AWS Trusted Advisor报告，以确定最近使用的服务。
- D. 移除默认的FullAWSAccess SCP。
- E. 定义组织单位（OUs），并将成员账户放置在OUs中。
- F. 移除默认的DenyAWSAccess SCP。
...
- A. 部署一个额外的CloudFront分发。为两个CloudFront分发创建一个新的Route 53故障转移记录集和健康检查。
- B. 将用于每个Region的后端服务的现有Route 53记录集的TTL设置为4秒。
- C. 使用延迟路由策略为后端服务创建新的记录集。将记录集用作CloudFront分发的起源。
- D. 创建一个包含两个起源的CloudFront起源组，一个用于每个后端服务Region。将起源故障转移配置为CloudFront分发的缓存行为。

---

### 单选题 274/306

一家公司在Amazon EC2实例后面部署了一个内网Web应用程序，这些实例位于一个应用程序负载均衡器（ALB）后面。目前，用户使用内部用户数据库对应用程序进行身份验证。公司需要使用现有的AWS Directory Service for Microsoft Active Directory目录对应用程序进行用户身份验证。目录中所有用户都必须能够访问应用程序。哪种解决方案将满足这些要求？

- A. 在目录中创建一个新的应用程序客户端。为ALB创建一个监听器规则。为监听器规则指定authenticate-oidc操作。使用ALB提供的回调URL配置新的应用程序客户端。
- B. 配置Amazon Cognito用户池。将用户池配置为具有来自目录的元数据的联合身份提供者（IdP）。创建一个应用程序客户端。将应用程序客户端与用户池关联。为ALB创建一个监听器规则。指定authenticate-cognito操作用于监听器规则。配置监听器规则使用用户池和应用程序客户端。
- C. 将目录添加为一个新的IAM身份提供者（IdP）。创建一个具有SAML 2.0联合实体类型的新IAM角色。配置角色策略以允许访问ALB。将新角色配置为IdP的默认经过身份验证的用户角色。为ALB创建一个监听器规则。指定authenticate-oidc操作用于监听器规则。
- D. 启用AWS IAM Identity Center（AWS Single Sign-On）。将目录配置为使用SAML的外部身份提供者。使用自动配置方法。创建一个具有SAML 2.0联合实体类型的新IAM角色。配置角色策略以允许访问ALB。将新角色附加到所有组。为ALB创建一个监听器规则。指定authenticate-cognito操作用于监听器规则。
...
- A. 在AWS组织中创建一个组织。设置AWS Control Tower，并打开强烈推荐的控制（护栏）。将所有账户加入到组织。将AWS账户分类到OUs中。
- B. 使用AWS CLI列出所有AWS账户中所有未加密的卷。运行一个脚本来就地加密所有未加密的卷。
- C. 为每个未加密的卷创建一个快照。从未加密的快照创建一个新的加密卷。分离现有卷，并用加密卷替换它。
- D. 在AWS组织中创建一个组织。设置AWS Control Tower，并打开强制性控制（护栏）。将所有账户加入到组织。将AWS账户分类到OUs中。
- E. 打开AWS CloudTrail。配置一个Amazon EventBridge规则以检测并自动加密未加密的卷。
...
- A. 在每个成员账户中创建OrganizationAccountAccess IAM组。包括每个管理员所需的IAM角色。
- B. 在每个成员账户中创建OrganizationAccountAccessPolicy IAM策略。使用跨账户访问将成员账户连接到管理账户。
- C. 在每个成员账户中创建OrganizationAccountAccessRole IAM角色。授予管理账户权限以承担IAM角色。
- D. 在管理账户中创建OrganizationAccountAccessRole IAM角色。附加AdministratorAccess AWS管理策略到IAM角色。将IAM角色分配给每个成员账户的管理员。
...
- A. 为API Gateway创建一个接口VPC终端节点。附加一个允许apigateway:*操作的终端节点策略。禁用VPC终端节点的私有DNS命名。配置API资源策略以允许来自VPC的访问。使用VPC终端节点的DNS名称访问API。
- B. 为API Gateway创建一个接口VPC终端节点。附加一个允许execute-api:Invoke操作的终端节点策略。启用VPC终端节点的私有DNS命名。配置API资源策略以允许来自VPC终端节点的访问。使用API终端节点的DNS名称访问API。
- C. 创建一个网络负载均衡器（NLB）和VPC链接。在API Gateway和NLB之间配置私有集成。使用API终端节点的DNS名称访问API。
- D. 创建一个应用负载均衡器（ALB）和VPC链接。在API Gateway和ALB之间配置私有集成。使用ALB终端节点的DNS名称访问API。
...
- A. 更新现有的VPC，并为VPC和所有子网关联自定义IPv6 CIDR块。更新所有VPC路由表，并为::/0添加路由到互联网网关。
- B. 更新现有的VPC，并为VPC和所有子网关联Amazon提供的IPv6 CIDR块。更新所有私有子网的VPC路由表，并为::/0添加路由到NAT网关。
- C. 更新现有的VPC，并为VPC和所有子网关联Amazon提供的IPv6 CIDR块。创建一个出口专用互联网网关。更新所有私有子网的VPC路由表，并为::/0添加路由到出口专用互联网网关。
- D. 更新现有的VPC，并为VPC和所有子网关联自定义IPv6 CIDR块。创建一个新的NAT网关，并启用IPv6支持。更新所有私有子网的VPC路由表，并为::/0添加路由到启用IPv6的NAT网关。
...
- A. 在us-east-1配置一个PrivateLink终端节点服务，使用现有的位于eu-west-2的NLB。授予特定AWS账户访问SaaS服务的权限。
- B. 在us-east-1创建一个NLB。创建一个IP目标组，使用公司在eu-west-2托管SaaS服务的实例的IP地址。配置一个使用位于us-east-1的NLB的PrivateLink终端节点服务。授予特定AWS账户访问SaaS服务的权限。
- C. 在eu-west-2的EC2实例前创建一个应用负载均衡器（ALB）。在us-east-1创建一个NLB。将位于us-east-1的NLB与使用位于eu-west-2的ALB的ALB目标组关联。配置一个使用位于us-east-1的NLB的PrivateLink终端节点服务。授予特定AWS账户访问SaaS服务的权限。
- D. 使用AWS资源访问管理器（AWS RAM）共享位于eu-west-2的EC2实例。在us-east-1，创建一个NLB和一个实例目标组，包括来自eu-west-2的共享EC2实例。配置一个使用位于us-east-1的NLB的PrivateLink终端节点服务。授予特定AWS账户访问SaaS服务的权限。
...
- A. 为数据科学家需要访问的每个包创建一个 AWS CodeCommit 存储库。配置 CodeCommit 存储库与 PyPI 存储库之间的代码同步。为 CodeCommit 创建一个 VPC 端点。
- B. 在 VPC 中创建一个 NAT 网关。配置 VPC 路由以允许通过网络 ACL 访问互联网，该网络 ACL 仅允许访问 PyPI 存储库端点。
- C. 在 VPC 中创建一个 NAT 实例。配置 VPC 路由以允许访问互联网。配置 SageMaker 笔记本实例防火墙规则，仅允许访问 PyPI 存储库端点。
- D. 创建一个 AWS CodeArtifact 域和存储库。向 CodeArtifact 存储库添加一个公共：pypi 的外部连接。配置 Python 客户端使用 CodeArtifact 存储库。为 CodeArtifact 创建一个 VPC 端点。
...
- A. 修改自动扩展组，设置更新策略以针对最旧的启动配置进行替换。
- B. 在下一个补丁维护窗口之前创建一个新的自动扩展组。在维护窗口期间，对两个组进行补丁并重新启动实例。
- C. 在自动扩展组前面创建一个 Elastic Load Balancer。配置监控，以确保在自动扩展组替换被终止的实例后，目标组健康检查返回健康状态。
- D. 创建自动化脚本，对 AMI 进行补丁，更新启动配置，并调用自动扩展实例刷新。
- E. 在自动扩展组前面创建一个 Elastic Load Balancer。在实例上配置终止保护。
...
- A. 使用分散放置组。为每个可用区设置至少八个实例的最小数量。
- B. 停止并重新启动放置组中的所有实例。再次尝试启动。
- C. 创建一个新的放置组。尝试将新放置组与原始放置组合并。
- D. 以放置组中的专用主机启动附加实例。
...
- A. 对于 API Gateway 方法，将授权设置为 AWS_IAM。然后，给 IAM 用户或角色在 REST API 资源上执行 execute-api:Invoke 权限。启用 API 调用者使用 AWS 签名在访问端点时签名请求。使用 AWS X-Ray 跟踪和分析 API Gateway 的用户请求。
- B. 对于 API Gateway 资源，将 CORS 设置为启用，并仅在 Access-Control-Allow-Origin 头中返回公司域名。然后，给 IAM 用户或角色在 REST API 资源上执行 execute-api:Invoke 权限。使用 Amazon CloudWatch 跟踪和分析 API Gateway 的用户请求。
- C. 创建一个 AWS Lambda 函数作为自定义授权器，要求 API 客户端在进行调用时传递密钥和密码，然后使用 Lambda 验证密钥/密码对与 IAM 系统。使用 AWS X-Ray 跟踪和分析 API Gateway 的用户请求。
- D. 为 API Gateway 创建客户端证书。将证书分发给需要访问端点的 AWS 用户和角色。启用 API 调用者在访问端点时传递客户端证书。使用 Amazon CloudWatch 跟踪和分析 API Gateway 的用户请求。
...
- A. 在分区放置组中启动内存优化的EC2实例。
- B. 在分区放置组中启动计算优化的EC2实例。
- C. 在集群放置组中启动内存优化的EC2实例。
- D. 在扩展放置组中启动计算优化的EC2实例。
...
- A. 创建一个新的AWS CloudTrail轨迹。使用组织管理账户中的现有Amazon S3存储桶来存储日志。在所有AWS Region部署该轨迹。在S3存储桶上启用MFA删除和加密。
- B. 在组织的每个成员账户中创建一个新的AWS CloudTrail轨迹。为存储日志创建新的Amazon S3存储桶。在所有AWS Region部署该轨迹。在S3存储桶上启用MFA删除和加密。
- C. 在组织的管理账户中创建一个新的AWS CloudTrail轨迹。创建一个新的带有版本控制的Amazon S3存储桶来存储日志。为组织中的所有账户部署该轨迹。在S3存储桶上启用MFA删除和加密。
- D. 在组织的管理账户中创建一个新的AWS CloudTrail轨迹。创建一个新的用于存储日志的Amazon S3存储桶。配置Amazon Simple Notification Service（Amazon SNS）将日志文件传递通知发送到一个外部管理系统，该系统将跟踪日志。在S3存储桶上启用MFA删除和加密。
...
- A. 使用迁移评估员从AWS请求对环境的评估。使用AWS应用发现服务无代理收集器将细节导入迁移评估员快速洞察报告。
- B. 使用AWS迁移中心并在服务器上安装AWS应用发现代理。部署迁移中心策略建议应用程序数据收集器。使用迁移中心策略建议生成报告。
- C. 使用AWS迁移中心并在服务器上运行AWS应用发现服务无代理收集器。使用AWS应用迁移服务对服务器和数据库进行分组。使用迁移中心策略建议生成报告。
- D. 使用AWS迁移中心导入工具加载公司本地环境的细节。使用迁移中心策略建议生成报告。
...
- A. 在其他可用区部署另一个NAT网关。使用适当的路由更新路由表。将RDS for MySQL数据库实例修改为Multi-AZ配置。配置自动扩展组在可用区跨区域启动实例。将自动扩展组的最小容量和最大容量设置为3。
- B. 用虚拟专用网关替换NAT网关。将RDS for MySQL数据库实例替换为Amazon Aurora MySQL数据库集群。配置自动扩展组在VPC的所有子网中启动实例。将自动扩展组的最小容量和最大容量设置为3。
- C. 用NAT实例替换NAT网关。将RDS for MySQL数据库实例迁移到RDS for PostgreSQL数据库实例。在其他可用区启动一个新的EC2实例。
- D. 在其他可用区部署另一个NAT网关。使用适当的路由更新路由表。将RDS for MySQL数据库实例设置为开启自动备份并保留备份7天。配置自动扩展组在VPC的所有子网中启动实例。保持自动扩展组的最小容量和最大容量为1。

---

### 单选题 288/306

一家科学公司需要处理来自亚马逊S3存储桶的文本和图像数据。这些数据是在深空任务的实时、时间紧迫阶段由几个雷达站收集的。雷达站将数据上传到源S3存储桶。数据以前缀标记雷达站的识别号码。公司在第二个账户中创建了一个目标S3存储桶。为了满足合规目标，必须将数据从源S3存储桶复制到目标S3存储桶。这种复制是通过使用S3复制规则实现的，以覆盖源S3存储桶中的所有对象。一个特定的雷达站被识别为具有最准确的数据。在雷达站将对象上传到源S3存储桶后，必须在30分钟内监控此雷达站的数据复制完成情况。解决方案架构师应该做些什么来满足这些要求？

- A. 设置AWS DataSync代理，以复制源S3存储桶中带有前缀的数据到目标S3存储桶。选择在此任务上使用所有可用带宽，并监控任务以确保它处于TRANSFERRING状态。创建Amazon EventBridge规则，以在该状态改变时启动警报。
- B. 在第二个账户中，为最准确的雷达站创建另一个S3存储桶以接收数据。为此新的S3存储桶设置一个新的复制规则，以将其他雷达站的复制分开。监控到目标的最大复制时间。创建Amazon EventBridge规则，在时间超过期望的阈值时启动警报。
- C. 在源S3存储桶上启用Amazon S3 Transfer Acceleration，并配置最准确的雷达站使用新的终端。监控S3目标存储桶的TotalRequestLatency指标。创建Amazon EventBridge规则，以在该状态改变时启动警报。
- D. 在源S3存储桶上创建一个新的S3复制规则，过滤使用最准确雷达站前缀的键。启用S3复制时间控制(S3 RTC)。监控到目标的最大复制时间。创建Amazon EventBridge规则，在时间超过期望的阈值时启动警报。
...
- A. 在 AWS 账户中创建一个传输网关。使用 AWS 资源访问管理器 (AWS RAM) 在账户之间共享传输网关。
- B. 配置所有 VPC 和 VPN 的连接。
- C. 设置传输网关路由表。将 VPC 和 VPN 与路由表关联。
- D. 配置 VPC 之间的对等连接。
...
- A. 在指定的非生产账户中实现跨账户备份与 AWS Backup 存储库。
- B. 添加一个 SCP，限制修改 AWS Backup 存储库。
- C. 实施 AWS Backup Vault Lock 合规模式。
- D. 实施分配给 AWS Backup 的 IAM 服务角色的最少权限访问。
- E. 配置备份频率、生命周期和保留期，确保在冷层中始终至少存在一个备份。
...
- A. 在 AWS Organizations 中创建一个组织。为所有账户创建一个单一的 SCP，以实现最少权限访问。为所有账户创建一个单一的 OU。为与本地 AD FS 服务器的联合配置 IAM 身份提供者。为中央日志账户配置一个定义的过程，用于日志生成服务将日志事件发送到中央账户。在中央账户中启用 AWS Config，并为所有账户配置合规包。
- B. 在 AWS Organizations 中创建一个组织。在组织上启用 AWS Control Tower。查看包括的控制（护栏）SCP。检查 AWS Config 需要增加的区域。必要时添加 OUs。将 AWS IAM Identity Center (AWS Single Sign-On) 连接到本地 AD FS 服务器。
- C. 在 AWS Organizations 中创建一个组织。为最少权限访问创建 SCP。创建 OU 结构，并使用它来组织 AWS 账户。将 AWS IAM Identity Center (AWS Single Sign-On) 连接到本地 AD FS 服务器。为中央日志账户配置一个定义的过程，用于日志生成服务将日志事件发送到中央账户。在中央账户中启用 AWS Config，并为所有账户配置聚合器和合规包。
- D. 在 AWS Organizations 中创建一个组织。在组织上启用 AWS Control Tower。查看包括的控制（护栏）SCP。检查 AWS Config 需要增加的区域。为与本地 AD FS 服务器的联合配置 IAM 身份提供者。

---

### 单选题 292/306

一家公司使用应用程序负载均衡器 (ALB) 和托管在 Amazon ECS 集群中的 Docker 应用程序托管社区论坛网站。网站数据存储在 Amazon RDS for MySQL 中，容器镜像存储在 ECR 中。公司需要为其客户提供灾难恢复 SLA，恢复时间目标 (RTO) 不超过 24 小时，恢复点目标 (RPO) 不超过 8 小时。以下哪种解决方案是最符合成本效益的方式来满足要求？

- A. 使用 AWS CloudFormation 在两个区域部署相同的 ALB、EC2、ECS 和 RDS 资源。每 8 小时安排 RDS 快照。使用 RDS 多区域复制更新辅助区域的数据库副本。在发生故障时，从最新快照恢复，并使用 Amazon Route 53 DNS 故障转移策略自动将客户重定向到辅助区域的 ALB。
- B. 在两个区域的 ECR 中存储 Docker 镜像。每 8 小时安排 RDS 快照，并将快照复制到辅助区域。在发生故障时，使用 AWS CloudFormation 在辅助区域部署 ALB、EC2、ECS 和 RDS 资源，从最新快照恢复，并更新 DNS 记录以指向辅助区域的 ALB。
- C. 使用 AWS CloudFormation 在辅助区域部署相同的 ALB、EC2、ECS 和 RDS 资源。每小时将 RDS MySQL 备份到 Amazon S3，并使用跨区域复制将数据复制到辅助区域的存储桶。在发生故障时，将最新 Docker 镜像导入辅助区域的 Amazon ECR，部署到 EC2 实例，恢复最新的 MySQL 备份，并更新 DNS 记录以指向辅助区域的 ALB。
- D. 在辅助区域部署一个试点环境，带有 ALB 和一个最小资源 EC2 部署用于 Docker 在 AWS Auto Scaling 组中的扩展策略，以增加实例大小和节点数量。创建 RDS 数据的跨区域读取副本。在发生故障时，提升副本为主副本，并更新 DNS 记录以指向辅助区域的 ALB。
...
- A. 在 AWS Organizations 中创建一个自定义 SCP，只允许部署突发性实例，并禁止使用不相关的服务。将 SCP 应用于开发 OU。
- B. 在 AWS Control Tower 中创建一个自定义侦探控制（护栏）。配置控制（护栏）以只允许部署突发性实例，并禁止使用不相关的服务。将控制（护栏）应用于开发 OU。
- C. 在 AWS Control Tower 中创建一个自定义预防控制（护栏）。配置控制（护栏）以只允许部署突发性实例，并禁止使用不相关的服务。将控制（护栏）应用于开发 OU。
- D. 在 AWS Control Tower 账户中创建一个 AWS Config 规则。配置 AWS Config 规则以只允许部署突发性实例，并禁止使用不相关的服务。使用 AWS CloudFormation StackSets 将 AWS Config 规则部署到开发 OU。

---

### 单选题 294/306

一家公司想要将其本地应用程序迁移到 AWS。应用程序的数据库存储结构化的产品数据和临时用户会话数据。公司需要将产品数据与用户会话数据解耦。公司还需要在另一个 AWS 区域实现复制以进行灾难恢复。哪种解决方案将以最高性能满足这些要求？

- A. 创建一个 Amazon RDS DB 实例，使用单独的模式来托管产品数据和用户会话数据。为 DB 实例在另一个区域配置读取副本。
- B. 创建一个 Amazon RDS DB 实例来托管产品数据。为 DB 实例在另一个区域配置读取副本。为用户会话数据创建一个 Amazon ElastiCache for Memcached 的全局数据存储。
- C. 创建两个 Amazon DynamoDB 全球表。使用一个全局表来托管产品数据。使用另一个全局表来托管用户会话数据。使用 DynamoDB Accelerator (DAX) 进行缓存。
- D. 创建一个 Amazon RDS DB 实例来托管产品数据。为 DB 实例在另一个区域配置读取副本。创建一个 Amazon DynamoDB 全球表来托管用户会话数据。

---

### 单选题 295/306

一家公司正在将其应用程序重新架构以在 AWS 上运行。公司的基础设施包括多个 Amazon EC2 实例。公司的开发团队需要不同级别的访问权限。公司希望实施一项政策，要求所有 Windows EC2 实例加入到 AWS 上的 Active Directory 域。公司还希望实施增强的安全流程，例如多因素认证 (MFA)。公司希望尽可能使用托管的 AWS 服务。哪种解决方案将满足这些要求？

- A. 创建一个 AWS 目录服务 Microsoft Active Directory 实现。启动一个 Amazon Workspace。连接到并使用 Workspace 进行域安全配置任务。
- B. 创建一个 AWS 目录服务 Microsoft Active Directory 实现。启动一个 EC2 实例。连接到并使用 EC2 实例进行域安全配置任务。
- C. 创建一个 AWS 目录服务 Simple AD 实现。启动一个 EC2 实例。连接到并使用 EC2 实例进行域安全配置任务。
- D. 创建一个 AWS 目录服务 Simple AD 实现。启动一个 Amazon Workspace。连接到并使用 Workspace 进行域安全配置任务。
...
- A. 为应用程序创建新的 Amazon DocumentDB（支持 MongoDB 兼容）表，并使用预配置 IOPS 卷。使用实例端点连接到 Amazon DocumentDB。
- B. 为应用程序创建新的 Amazon DynamoDB 表，并使用按需容量。使用 DynamoDB 的网关 VPC 端点连接到 DynamoDB 表。
- C. 为应用程序创建新的 Amazon DynamoDB 表，并使用按需容量。使用 DynamoDB 的接口 VPC 端点连接到 DynamoDB 表。
- D. 为应用程序创建新的 Amazon DocumentDB（支持 MongoDB 兼容）表，并使用预配置 IOPS 卷。使用集群端点连接到 Amazon DocumentDB。
...
- A. 启动新的 EC2 实例，并为每个实例生成一个单独的 SSH 密钥。将 SSH 密钥存储在 AWS Secrets Manager 中。创建一个新的 IAM 策略，并将其附加到工程师的 IAM 角色中，允许执行 GetSecretValue 操作。指示工程师在通过任何 SSH 客户端连接时从 Secrets Manager 获取 SSH 密钥。
- B. 创建一个 AWS Systems Manager 文档，以在 EC2 实例上运行命令设置一个新的唯一 SSH 密钥。创建一个新的 IAM 策略，并将其附加到工程师的 IAM 角色中，允许执行 Systems Manager 文档。指示工程师运行文档设置 SSH 密钥，并通过任何 SSH 客户端连接。
- C. 启动新的 EC2 实例，不为实例设置任何 SSH 密钥。在每个实例上设置 EC2 实例连接。创建一个新的 IAM 策略，并将其附加到工程师的 IAM 角色中，允许执行 SendSSHPublicKey 操作。指示工程师使用 EC2 控制台中的基于浏览器的 SSH 客户端连接到实例。
- D. 设置 AWS Secrets Manager 以存储 EC2 SSH 密钥。创建一个新的 AWS Lambda 函数以创建一个新的 SSH 密钥，并调用 AWS Systems Manager Session Manager 将 SSH 密钥设置在 EC2 实例上。配置 Secrets Manager 使用 Lambda 函数每天自动轮换一次。指示工程师在通过任何 SSH 客户端连接时从 Secrets Manager 获取 SSH 密钥。
...
- A. 创建一个 Amazon EventBridge 规则以检测未加密 EBS 卷的创建。调用 AWS Lambda 函数以删除不符合规定的卷。
- B. 使用 AWS Audit Manager 和数据加密。
- C. 创建一个 AWS Config 规则以检测新 EBS 卷的创建。使用 AWS Systems Manager 自动化加密该卷。
- D. 在所有 AWS 区域中默认开启 EBS 加密。
...
- A. 使用 AWS Directory Service for Microsoft Active Directory 部署托管的 Active Directory。与本地 Active Directory 建立信任。在 VPC 中部署一个 EC2 实例作为堡垒主机。确保 EC2 实例加入到域中。使用堡垒主机通过 RDP 访问目标实例。
- B. 通过使用 AWS Directory Service for Microsoft Active Directory AD Connector 将 AWS IAM Identity Center (AWS Single Sign-On) 配置为与本地 Active Directory 集成。针对用户组配置权限集以访问 AWS Systems Manager。使用 Systems Manager Fleet Manager 通过 RDP 访问目标实例。
- C. 在本地环境和目标 VPC 之间实现 VPN。确保目标实例通过 VPN 连接加入到本地 Active Directory 域。通过 VPN 配置 RDP 访问。从公司的网络连接到目标实例。
- D. 使用 AWS Directory Service for Microsoft Active Directory 部署托管的 Active Directory。与本地 Active Directory 建立信任。使用 AWS Quick Start 在 AWS 上部署远程桌面网关。确保远程桌面网关加入到域中。使用远程桌面网关通过 RDP 访问目标实例。
...
- A. 将 API 部署到多个区域。使用自定义域名配置 Amazon Route 53，以将流量路由到每个区域性 API 端点。实施 Route 53 多值应答路由策略。
- B. 创建一个新的 KMS 多区域客户管理密钥。在每个相关区域创建一个新的 KMS 客户管理复制密钥。
- C. 将现有的 Secrets Manager 密钥复制到其他区域。对于每个相关区域的复制密钥，选择适当的 KMS 密钥。
- D. 在每个相关区域创建一个新的 AWS 管理 KMS 密钥。将现有密钥转换为多区域密钥。在其他区域使用多区域密钥。
- E. 在每个相关区域创建一个新的 Secrets Manager 密钥。从现有区域的密钥复制密钥值到每个新密钥。
- F. 修改 Lambda 函数的部署流程，以在相关区域重复部署。为现有 API 打开多区域选项。选择在每个区域部署的 Lambda 函数作为多区域 API 的后端。

### 多选题 301/306

一家公司正在部署来自AWS Marketplace的第三方防火墙设备解决方案，以监控和保护离开公司AWS环境的流量。公司希望将这些设备部署到共享服务VPC中，并将所有出站的互联网流量路由到这些设备。解决方案架构师需要推荐一种部署方法，优先考虑可靠性，并最小化单个AWS区域内防火墙设备之间的故障转移时间。公司已经从共享服务VPC路由到其他VPC。解决方案架构师应该推荐哪些步骤来满足这些要求？（选择三个）

- A. 在共享服务VPC中部署两个防火墙设备，每个在不同的可用区。
- B. 在共享服务VPC中创建一个新的网络负载均衡器。创建一个新的目标组，并将新网络负载均衡器附加到目标组。将每个防火墙设备实例添加到目标组。
- C. 在共享服务VPC中创建一个新的网关负载均衡器。创建一个新的目标组，并将新网关负载均衡器附加到目标组。将每个防火墙设备实例添加到目标组。
- D. 创建一个VPC接口端点。向共享服务VPC中的路由表添加路由。将新端点指定为从其他VPC进入共享服务VPC的流量的下一跳。
- E. 在共享服务VPC中部署两个防火墙设备，每个在同一个可用区。
- F. 创建一个VPC网关负载均衡器端点。向共享服务VPC中的路由表添加路由。将新端点指定为从其他VPC进入共享服务VPC的流量的下一跳。

---

### 多选题 302/306

一家公司使用AWS组织和多账户架构。公司目前的账户安全配置包括SCP、基于资源的策略、基于身份的策略、信任策略和会话策略。解决方案架构师需要允许账户A中的IAM用户假定账户B中的角色。解决方案架构师必须采取哪三个步骤来满足此要求？

- A. 为账户A配置SCP以允许该操作。
- B. 为资源配置基于资源的策略以允许该操作。
- C. 为账户A中的用户配置基于身份的策略以允许该操作。
- D. 为账户B中的用户配置基于身份的策略以允许该操作。
- E. 为账户B中的目标角色配置信任策略以允许该操作。
- F. 配置会话策略以允许该操作，并可通过GetSessionToken API操作以编程方式传递。
...
- A. 打开AWS备份的跨账户管理功能。创建一个备份计划，指定频率和保留要求。给DB实例添加标签。使用标签应用备份计划。使用AWS备份监控备份状态。
- B. 打开Amazon RDS的跨账户管理功能。创建一个快照全局策略，指定频率和保留要求。使用管理账户中的RDS控制台监控备份状态。
- C. 打开AWS CloudFormation的跨账户管理功能。从管理账户部署一个CloudFormation堆栈集，其中包含来自AWS备份的备份计划，指定频率和保留要求。在管理账户中创建一个AWS Lambda函数，以监控备份状态。在每个账户中创建一个Amazon EventBridge规则，按计划运行Lambda函数。
- D. 在每个账户中配置AWS备份。创建一个Amazon Data Lifecycle Manager生命周期策略，指定频率和保留要求。将DB实例指定为目标资源。使用每个成员账户中的Amazon Data Lifecycle Manager控制台监控备份状态。
...
- A. 预配置一个Amazon EMR集群，卸载复杂的数据处理任务。
- B. 部署一个AWS Lambda函数，在集群的CPU指标在Amazon CloudWatch中达到80%时，使用经典调整大小操作向Amazon Redshift集群添加容量。
- C. 部署一个AWS Lambda函数，在集群的CPU指标在Amazon CloudWatch中达到80%时，使用弹性调整大小操作向Amazon Redshift集群添加容量。
- D. 打开Amazon Redshift集群的并发扩展功能。
...
- A. 创建私有托管区域。激活VPC的enableDnsSupport属性和enableDnsHostnames属性。更新VPC的DHCP选项集，以包括domain-name-servers=10.24.34.2。
- B. 创建私有托管区域。将私有托管区域与VPC关联。激活VPC的enableDnsSupport属性和enableDnsHostnames属性。创建新的VPC DHCP选项集，并配置domain-name-servers=AmazonProvidedDNS。将新的DHCP选项集与VPC关联。
- C. 停用VPC的enableDnsSupport属性。激活VPC的enableDnsHostnames属性。创建新的VPC DHCP选项集，并配置domain-name-servers=10.24.34.2。将新的DHCP选项集与VPC关联。
- D. 创建私有托管区域。将私有托管区域与VPC关联。激活VPC的enableDnsSupport属性。停用VPC的enableDnsHostnames属性。更新VPC的DHCP选项集，以包括domain-name-servers=AmazonProvidedDNS。
...
- A. 创建一个专用于数据处理微服务的对称AWS Key Management Service（AWS KMS）密钥。创建一个字段级加密配置文件和配置。将KMS密钥和配置与CloudFront缓存行为关联。
- B. 创建一个专用于数据处理微服务的RSA密钥对。将公钥上传到CloudFront分发。创建一个字段级加密配置文件和配置。将配置添加到CloudFront缓存行为。
- C. 创建一个专用于数据处理微服务的对称AWS Key Management Service（AWS KMS）密钥。创建一个Lambda@Edge函数。编写函数程序，使用KMS密钥对敏感数据进行加密。
- D. 创建一个专用于数据处理微服务的RSA密钥对。创建一个Lambda@Edge函数。编写函数程序，使用RSA密钥对的私钥对敏感数据进行加密。
